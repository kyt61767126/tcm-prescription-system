...
export async function onRequest(context) {
    ...
    try {
        ...
        if (existingDevice) {
            ...
            // ★ 阶段2 存量回填：旧设备记录无端形态时补空（与 heartbeat 嗅探兜底
            //   同语义：仅补空不覆盖），提升阶段3 切换时标识覆盖率
            if (!existingDevice.clientClass && cClass) existingDevice.clientClass = cClass;
            if (!existingDevice.productClass && pClass) existingDevice.productClass = pClass;
        } else {
            // 新设备激活：添加到数组
            newDevices.push({
                machineId: machineId,
