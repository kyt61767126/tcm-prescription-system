...
async function saveLicense(licenseBase64) {
    try {
        ...
        // 同步 clinicName、doctorName 到 config.json + 创建管理员账户
        try {
            ...
            if (savedPhone && savedPassword) {
                ...
                // 检查是否已存在同手机号用户
                const existing = config.users.find(u => u.username === savedPhone);
                if (!existing) {
                    // 密码哈希：SHA256(SALT + password)
                    const crypto = require('crypto');
                    const SALT = 'tcm_prescription_2024';
                    const hashedPwd = crypto.createHash('sha256').update(SALT + savedPassword).digest('hex');
                    config.users.push({
                        username: savedPhone,
                        password: hashedPwd,
                        name: savedAdminName,
                    ...
                    console.log('[Activate] 管理员账户已存在:', savedPhone);
                }
            }
            ...
            console.warn('[Activate] 同步 config.json 失败:', e.message);
        }
