...
//       再跑生成器）。
//    4. T22 是已知欠债（escapeJs 语法错误条目），B2 阶段删除。
//
...
module.exports = [
  ...
  {
    id: "T22",
    desc: "【已知欠债·勿扩散】escapeJs 语法错误：addMediaFromAlbum 参数 prescriptionNo || '} 缺闭合引号——APP 侧历史 bug，B2 阶段将删除本条让桌面权威源修复流灌（删除后跑 sync-index-app.cjs 即修复）",
    find: "            let html = items.map((p,i) =>`<div class=\"history-item\" onclick=\"loadHistory(${Number(p.id)||0})\"><div style=\"display:flex;justify-content:space-between;align-items:center;\"><div><div style=\"display:flex;gap:8px;align-items:center;\"><div class=\"history-name\">${escapeHtml(p.patientName)}</div><div style=\"font-size:10px;color:#8b0000;font-weight:bold;\">[${escapeHtml(p.prescriptionNo || '')}]</div></div><div class=\"history-date\">${escapeHtml(p.date)} - ${escapeHtml(p.diagnosis || '')}${_rlFee(p)}</div></div><button onclick=\"event.stopPropagation();addMediaFromAlbum('${escapeJs(p.patientName)}','${escapeJs(p.prescriptionNo || '')}','${escapeJs(p.createdAt || '')}')\" style=\"background:none;border:none;font-size:14px;cursor:pointer;padding:0 2px;\" title=\"从相册添加照片视频\">📁</button><button onclick=\"event.stopPropagation();viewMediaFiles('${escapeJs(p.patientName)}','${escapeJs(p.prescriptionNo || '')}','${escapeJs(p.createdAt || '')}')\" style=\"background:none;border:none;font-size:14px;cursor:pointer;padding:0 2px;\" title=\"查看拍照录像\">📷🎥</button><button class=\"small-btn\" style=\"background:#ffdddd;padding:2px 6px;font-size:10px;\" onclick=\"event.stopPropagation();deleteHistory(${Number(p.id)||0})\">删除</button></div></div>`).join('');",
    replace: "            let html = items.map((p,i) =>`<div class=\"history-item\" onclick=\"loadHistory(${Number(p.id)||0})\"><div style=\"display:flex;justify-content:space-between;align-items:center;\"><div><div style=\"display:flex;gap:8px;align-items:center;\"><div class=\"history-name\">${escapeHtml(p.patientName)}</div><div style=\"font-size:10px;color:#8b0000;font-weight:bold;\">[${escapeHtml(p.prescriptionNo || '')}]</div></div><div class=\"history-date\">${escapeHtml(p.date)} - ${escapeHtml(p.diagnosis || '')}${_rlFee(p)}</div></div><button onclick=\"event.stopPropagation();addMediaFromAlbum('${escapeJs(p.patientName)}','${escapeJs(p.prescriptionNo || ')}','${escapeJs(p.createdAt || ')}')\" style=\"background:none;border:none;font-size:14px;cursor:pointer;padding:0 2px;\" title=\"从相册添加照片视频\">📁</button><button onclick=\"event.stopPropagation();viewMediaFiles('${escapeJs(p.patientName)}','${escapeJs(p.prescriptionNo || '')}','${escapeJs(p.createdAt || '')}')\" style=\"background:none;border:none;font-size:14px;cursor:pointer;padding:0 2px;\" title=\"查看拍照录像\">📷🎥</button><button class=\"small-btn\" style=\"background:#ffdddd;padding:2px 6px;font-size:10px;\" onclick=\"event.stopPropagation();deleteHistory(${Number(p.id)||0})\">删除</button></div></div>`).join('');",
  },
  {
    id: "T23",
