...
function Show-Menu {
    ...
    Write-Host "  [a] 快速全部打包 (跳过编码检查/配置编辑)" -ForegroundColor Green
    ...
    Write-Host "      说明: 把最新的 index.html 等文件复制到 APP 工程目录"
    Write-Host "      适用: 改了网页代码但还没准备好打包"
    Write-Host ""
    Write-Host "  [5] 仅修改诊所配置" -ForegroundColor White
    Write-Host "      说明: 修改诊所名称、医生姓名、地址等信息"
    Write-Host "      适用: 给新诊所定制版本时使用"
    Write-Host ""
    Write-Host "  [6] 仅编码检查"
    ...
    Write-Host ""
    Write-Host "  [7] 查看当前配置"
    Write-Host "      说明: 查看当前诊所名称、医生姓名等配置信息"
    Write-Host ""
    ...
    Write-Host "    • 只改了诊所信息: 选 [5] 修改配置，然后选 [9]" -ForegroundColor White
