...
# 离线系走 sync-index-app.cjs 独立生成链，见 tools/sync-index-app.cjs）
$Authority = 'public/index.html'
# ★ 2026-09-13 P1 收口：云APP assets 重新收编（2026-09-10 因"手机/桌面布局冲突"隔离，
#   现 public 权威源已含 .dose-mobile/.dose-desktop 双实例响应式（窄屏自动切手机布局），
#   冲突根源消解）。云APP 是 WebView 壳实载线上 public/（MainActivity 明示不加载本地
#   assets，断网走原生错误页），本地 assets 仅打包兜底——端配置块与 public 同值
#   （EDITION='personal'/APP_MODE='auto'）属有意保留：兜底内容与线上体验一致。
$Targets = @(
