...
echo.
echo   【⚠️ 重要：本工具只清本地，云端授权记录不会清！】
echo   云端按「设备识别码」（主板+CPU硬件指纹，同一台电脑
echo   离线版/云端版共用一个识别码）记忆激活记录。
echo   重置后如果打开「管理员激活」窗口，软件会自动恢复
echo   本机最近一次已激活的诊所（登录页显示旧诊所名，
echo   这是「断链自愈」设计，正常客户换机重装的保障）。
echo.
echo   要彻底模拟全新客户，二选一：
echo   ① 管理后台删除该设备的激活申请记录
echo     （激活审核页删除该申请 + 诊所管理删除该诊所）
echo   ② 换一台没激活过的电脑测试
echo ============================================================
echo.
pause