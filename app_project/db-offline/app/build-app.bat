...
echo [7/10] 代码混淆（target=dingzhi）+ Java 预编译检查...
...
call node "%~dp0..\..\..\tools\obfuscate.js" restore --target=dingzhi
if errorlevel 1 (
    echo       [警告] JS 恢复失败 - 可能需要手动恢复: node tools\obfuscate.js restore --target=dingzhi
) else (
    echo       [OK] JS 已恢复到原始状态
)
echo.
