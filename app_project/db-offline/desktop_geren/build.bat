...
REM 记录开始时间（用于耗时统计），用 PowerShell 替代废弃的 wmic（Win11 已弃用）
...
powershell -NoProfile -Command "[Console]::OutputEncoding=[System.Text.Encoding]::UTF8; Get-Process | Where-Object { try { $_.Path -like '*db-offline\desktop_geren\dist*' -or $_.Path -like '*db-offline\desktop_geren\build_output*' } catch { $false } } | Stop-Process -Force -ErrorAction SilentlyContinue" 2>nul
powershell -NoProfile -Command "[Console]::OutputEncoding=[System.Text.Encoding]::UTF8; Write-Host '[OK] 残留进程已清理'"
echo.

powershell -NoProfile -Command "[Console]::OutputEncoding=[System.Text.Encoding]::UTF8; Write-Host '[4/9] 配置诊所信息...'"
if /i "%1"=="--skip-config" (
...
powershell -NoProfile -Command "[Console]::OutputEncoding=[System.Text.Encoding]::UTF8; Write-Host '[5/9] 清理旧打包产物 + 版本号自增...'"
set "OUTPUT_DIR=dist"
...
if exist "%OUTPUT_DIR%" (
    ...
    if exist "%OUTPUT_DIR%" (
        ...
        if exist "%OUTPUT_DIR%" (
            ...
            powershell -NoProfile -Command "[Console]::OutputEncoding=[System.Text.Encoding]::UTF8; Write-Host '请手动关闭使用 %OUTPUT_DIR%\ 的程序后重试'"
            powershell -NoProfile -Command "[Console]::OutputEncoding=[System.Text.Encoding]::UTF8; Write-Host '或手动删除/重命名 %OUTPUT_DIR% 文件夹'"
            if not defined NO_PAUSE pause
            exit /b 1
        )
    )
...
echo.
powershell -NoProfile -Command "[Console]::OutputEncoding=[System.Text.Encoding]::UTF8; Write-Host '版本号自增（触发完整性基线重建）...'"
...
powershell -NoProfile -Command "[Console]::OutputEncoding=[System.Text.Encoding]::UTF8; Write-Host '[7/9] 代码混淆（target=geren，可能耗时 1-2 分钟）...'"
...
powershell -NoProfile -Command "[Console]::OutputEncoding=[System.Text.Encoding]::UTF8; Write-Host '[8/9] 执行打包（npm run build + 失败重试）...'"
set ELECTRON_MIRROR=https://registry.npmmirror.com/-/binary/electron/
...
if not "%BUILD_RC%"=="0" (
    echo.
    powershell -NoProfile -Command "[Console]::OutputEncoding=[System.Text.Encoding]::UTF8; Write-Host '[警告] 首次构建失败，3 秒后重试...'"
...
if not "%BUILD_RC%"=="0" (
    echo.
    powershell -NoProfile -Command "[Console]::OutputEncoding=[System.Text.Encoding]::UTF8; Write-Host '[错误] 构建失败，请查看上方日志'"
...
powershell -NoProfile -Command "[Console]::OutputEncoding=[System.Text.Encoding]::UTF8; Write-Host '产物完整性验证...'"
set "EXE_FILE="
...
if errorlevel 1 (
    powershell -NoProfile -Command "[Console]::OutputEncoding=[System.Text.Encoding]::UTF8; Write-Host '[错误] 恢复原始代码失败！源代码可能仍处于混淆状态。'"
    powershell -NoProfile -Command "[Console]::OutputEncoding=[System.Text.Encoding]::UTF8; Write-Host '请手动执行: node "%~dp0..\..\..\tools\obfuscate.js" restore --target=geren'"
    if not defined NO_PAUSE pause
    exit /b 1
)
...
echo ============================================
if exist "dist_old_*" (
    powershell -NoProfile -Command "[Console]::OutputEncoding=[System.Text.Encoding]::UTF8; Write-Host '[提示] 旧打包产物已保存为 dist_old_* 目录'"
    powershell -NoProfile -Command "[Console]::OutputEncoding=[System.Text.Encoding]::UTF8; Write-Host '未来构建时将自动清理（仅保留最近 2 个）'"
)
