...
powershell -NoProfile -Command "[Console]::OutputEncoding=[System.Text.Encoding]::UTF8; Write-Host '[7/9] 代码混淆（target=dingzhi，可能耗时 1-2 分钟）...'"
...
if errorlevel 1 (
    ...
    powershell -NoProfile -Command "[Console]::OutputEncoding=[System.Text.Encoding]::UTF8; Write-Host '请手动执行: node "%~dp0..\..\..\tools\obfuscate.js" restore --target=dingzhi'"
    if not defined NO_PAUSE pause
    exit /b 1
)
