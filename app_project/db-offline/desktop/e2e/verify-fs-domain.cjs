// ============================================================================
// verify-fs-domain.cjs — B2-1 文件域抽取（desktop-fs-ipc.cjs）专项 E2E 验证
//
// 与 run-e2e.cjs 的区别：
...
// 离线桌面试用态 config（app 启动会自动补签名 + 试用期降级，与 run-e2e E1 同构）
function baseConfig() {
    return {
...
// ============================================================================
(async () => {
    console.log('[FSV] ══ B2-1 文件域抽取专项验证（dev electron + 源码直启）══');
    ...
    try {
        ...
        // 等权限流程跑完（改密按钮亮起 = edition/permission 已生效，renderer 完全可用）
    ...
    if (passed) {
        ...
        console.log('[FSV] 验证对象：shared/desktop-fs-ipc.cjs 经 sync-all 分发后的离线桌面副本');
        process.exit(0);
    } else {
