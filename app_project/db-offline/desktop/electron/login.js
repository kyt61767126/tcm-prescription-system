...
(function () {
    ...
    // ★ 2026-08-28 全局统一：桌面版登录框多账户下拉切换
    // ★ 2026-08-28 实名防护：所有 merged 构建入口 + 显示文本 全链路应用 isGenericUsername
    ...
    // ★ 2026-08-28 实名防护：只有 isGenericUsername 判定为"通用用户名"才写入记忆；真实手机号/医师名/邮箱一律清空记忆键
    ...
    async function handleLogin() {
        ...
        try {
            ...
            //   点确定只留页红字提示，已填手机号/密码保留（按钮由 finally 复位），文案与
            //   主窗/APP 零账户守卫一致。避免老用户或数据暂未读到的场景被强制弹窗。
            if (!_users || _users.length === 0) {
                showError('⚠️ 本机尚未注册管理员账户，请点击【注册开通】完成注册（注册后即可免费试用）');
                return;
            }
            ...
            try {
                ...
                if (machineIsInstitution !== accountIsInstitution) {
                    ...
                    //   ①本机功能（用户管理等）由本机激活码决定——permission 按 config.edition、
                    ...
                    if (accountIsInstitution && !machineIsInstitution) {
                        user._editionNote = '。提示：本机为标准版授权，机构版专属功能不可用，可激活机构版解锁';
                    }
