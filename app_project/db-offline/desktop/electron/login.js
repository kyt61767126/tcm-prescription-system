...
(function () {
    ...
    function checkFirstRun(config) {
        ...
        if (window.electronAPI && window.electronAPI.bnzcGetPendingActivation) {
            // 注意：此处为同步检查标记，实际数据在 DOMContentLoaded 开头已获取
            // 若待激活数据尚未清除，跳过向导
    ...
    async function finishWizard() {
        ...
        if (clinicName && window.electronAPI) {
            try {
                ...
                console.log('[Wizard] 诊所信息已保存:', clinicName);
        ...
        msg += '\n\n请使用' + (pwdChanged ? '新密码' : '默认密码 admin') + '登录开始试用';
        alert(msg);
    }
