...
function Invoke-EncodingCheck {
    ...
    $htmlFiles = @(
        "$scriptDir\db-yudan/cloud_desktop\index.html",
        "$scriptDir\db-yudan/cloud_app\app\src\main\assets\public\index.html"
    ) | Where-Object { Test-Path $_ }
...
# ============================================================================
# Section 3: File Sync (shared/ -> db-yudan/cloud_app，对齐离线版 Sync-FilesToApp)
# ============================================================================

function Sync-FilesToCloudApp {
    Write-Step "同步文件到 db-yudan/cloud_app"
    Write-Host "  Syncing shared files to db-yudan/cloud_app assets..." -ForegroundColor White
    Write-Host ""

    $sharedDir = "$scriptDir\..\shared"
    $publicDir = "$scriptDir\db-yudan/cloud_app\app\src\main\assets\public"
    $assetsDir = "$scriptDir\db-yudan/cloud_app\app\src\main\assets"
...
function Show-Menu {
    ...
    Write-Host "  [5] 仅同步文件到 db-yudan/cloud_app"
...
function Edit-CloudConfig {
    ...
    # ----- 读取当前配置 -----
    $capFile = Join-Path $scriptDir 'db-yudan/cloud_app\app\src\main\assets\capacitor.config.json'
    $pkgFile = Join-Path $scriptDir 'db-yudan/cloud_desktop\package.json'

    if (-not (Test-Path $capFile)) {
        Write-Host "  [错误] 未找到 capacitor.config.json: $capFile" -ForegroundColor Red
        Write-Host "  请确认 db-yudan/cloud_app 目录结构完整" -ForegroundColor White
        return 1
    }
    if (-not (Test-Path $pkgFile)) {
        Write-Host "  [错误] 未找到 db-yudan/cloud_desktop/package.json: $pkgFile" -ForegroundColor Red
        return 1
    }
    ...
    # ----- 写入 db-yudan/cloud_desktop/package.json -----
...
function Confirm-BuildConfig {
    ...
    # 从 db-yudan/cloud_desktop/package.json 读取
    $pkgFile = Join-Path $scriptDir 'db-yudan/cloud_desktop\package.json'
    ...
    $gradleFile = Join-Path $scriptDir 'db-yudan/cloud_app\app\build.gradle'
...
function Build-Desktop {
    ...
    $stepStart = Get-Date
    $desktopDir = "$scriptDir\db-yudan/cloud_desktop"
    ...
    try {
        ...
        Get-Process | Where-Object { try { $_.Path -like "*db-yudan/cloud_desktop*dist*" } catch { $false } } | Stop-Process -Force -ErrorAction SilentlyContinue
        Write-Host "        [OK]"
...
function Build-App {
    ...
    Write-Host "  将执行以下步骤："
    Write-Host "  1. 同步 shared 文件到 db-yudan/cloud_app"
    ...
    $cloudAppDir = Join-Path $scriptDir 'db-yudan/cloud_app'
...
function Build-All {
    ...
    Write-Host ""
    Write-Host "  输出："
    Write-Host "  - 桌面版: db-yudan/cloud_desktop\dist\*.exe"
    ...
    Write-Host "================================================================" -ForegroundColor Yellow
    Write-Host ""
    Write-Host "  桌面版: $scriptDir\db-yudan/cloud_desktop\dist\" -ForegroundColor Yellow
...
function Show-Config {
    ...
    # 从 db-yudan/cloud_desktop/package.json 读取 productName
    $pkgFile = Join-Path $scriptDir 'db-yudan/cloud_desktop\package.json'
    if (Test-Path $pkgFile) {
        try {
            $pkg = Get-Content $pkgFile -Raw -Encoding UTF8 | ConvertFrom-Json
        ...
        Write-Host "  [警告] 未找到 db-yudan/cloud_desktop/package.json" -ForegroundColor Yellow
    }

    # 从 db-yudan/cloud_app/app/build.gradle 读取 applicationId 和 versionName
    $gradleFile = Join-Path $scriptDir 'db-yudan/cloud_app\app\build.gradle'
    if (Test-Path $gradleFile) {
        Write-Host ""
    ...
    $guardFile = Join-Path $scriptDir 'db-yudan/cloud_app\app\src\main\java\com\tcm\prescription\SecurityGuard.java'
...
function Build-AllStrict {
    ...
    Write-Host "================================================================" -ForegroundColor Green
    Write-Host ""
    Write-Host "  桌面版: $scriptDir\db-yudan/cloud_desktop\dist\" -ForegroundColor Green
...
function Build-AppStrict {
    ...
    $appDir = Join-Path $scriptDir "db-yudan/cloud_app"
