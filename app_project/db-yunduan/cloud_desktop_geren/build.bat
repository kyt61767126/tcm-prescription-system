...
cd /d "%~dp0"
title Huikang TCM Cloud (Personal) - Desktop Build
...
echo ============================================
echo  Huikang TCM Cloud (Personal) - Desktop Build
...
powershell -NoProfile -Command "Get-Process | Where-Object { try { $_.Path -like '*db-yunduan/cloud_desktop_geren*dist*' } catch { $false } } | Stop-Process -Force -ErrorAction SilentlyContinue" 2>nul
