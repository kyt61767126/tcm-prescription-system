// ============================================================================
// verify-windows-domain.cjs — B2-2 窗口域抽取（desktop-windows.cjs）专项 E2E 验证·云桌面
//
// 与 db-offline/desktop/e2e/verify-windows-domain.cjs 同构（双端窗口域字节级同体，
// 本脚本验证云桌面 main.js 集成点：require 工厂注入 + 启动链路 + 6 函数接线）。
// 差异仅三处：config（cloud_clinic 机构版）、登录后等待标志（userManageBtn）、标题。
//
...
// 云桌面机构版 config（app 启动会自动补签名；本地账号走离线校验 fallback，与 verify-fs-domain 同构）
function baseConfig() {
    return {
...
// ============================================================================
(async () => {
    console.log('[WV] ══ B2-2 窗口域抽取专项验证·云桌面（dev electron + 源码直启）══');
