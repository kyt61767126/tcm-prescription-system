...
set "PROJECT_DIR=%~dp0db-yunduan/cloud_app"
...
REM Read __APP_VERSION__ from db-yunduan/cloud_desktop/index.html, inject into MainActivity.EXPECTED_APP_VERSION
...
echo [4.5/6] Obfuscating JavaScript (cloud target - includes db-yunduan/cloud_app assets)...
...
set "PRODUCT_NAME="
for /f "delims=" %%p in ('powershell -NoProfile -Command "(Get-Content '..\db-yunduan/cloud_desktop\package.json' -Encoding UTF8 -Raw | ConvertFrom-Json).build.productName"') do (
    set "PRODUCT_NAME=%%p"
)
