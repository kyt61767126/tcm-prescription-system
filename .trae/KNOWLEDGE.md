...
### 2.104【优化】激活有效期统一从激活当天起算——固定日期码起算歧义（提交 a1c6bcdb，2026-08-26）
- **用户规则**：同一设备激活一次=完整365天，从**激活当天**算，而不是从第一天（码生成/首次使用日）算。
- **全局检查结论**（四端激活全汇聚服务端 `license-core.buildLicenseData`）：days 码（管理台/工单审批主路径）本就从激活时刻起算✅；**固定日期码（生成时指定 expiresAt）直接沿用固定到期日**——与激活日无关，激活后实际可用<365天（用户遇到的就是它）；云端诊所授权（users.js 自愈/创建/转正/续费）均从当下起算✅。
- **修复**：buildLicenseData 统一 `now + record.days(默认365)` 天；record.expiresAt 降级为①激活码使用期限（validate.js 过期校验保留）②续费叠加保底（取更晚者，extend.js 续费语义不破坏）。Node 单测 6/6（days365=365/默认365/固定剩30→365/固定剩400→400/续费剩500→500/trial7=7）。
- **坑**：record.expiresAt 是**双重语义字段**（码使用期限 vs license到期日）——旧代码两条分支都用它，直接删分支会破坏 extend.js 续费叠加，必须保留"取更晚者"保底。
- **生效方式**：push GitHub 自动部署，全端下次激活（含重装重激活）即生效；存量 license.dat 不变，到期后重新激活得完整365天。
