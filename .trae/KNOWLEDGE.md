...
★ 2026-09-05 admin-submit 换机自动解绑补齐（Commit 062ff589）：原激活本人换设备走「管理员激活→填手机号→提交」被 `ALREADY_ACTIVATED_OTHER_DEVICE` 硬拦截，只能联系客服手动解绑。修复：在 `!_isOwnerDevice` 分支加 phoneVerified 判定（提交手机号 == 原激活手机号）→ 从 license KV 记录读 devices/maxDevices → 达上限自动解绑最旧设备（与 validate.js L265 逻辑完全一致）→ updateLicense 回写 + buildLicenseData+encodeLicenseBase64 重签 + updateAdminRequestStatus 同步 admin_req → 继续短路复用下发新 license。phoneVerified=false（冒用他人手机号）保持 409 拒绝不扩大放行面（09-03 P0 安全修复纪律）。**铁律：admin-submit 和 validate.js 的换机逻辑必须对齐**——同本人换机两条路径都应该成功，不应一条走自动解绑一条走硬拦截。
