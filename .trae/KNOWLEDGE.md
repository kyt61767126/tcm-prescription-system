...
* ★ 2026-09-22 **官网清晰度优化（字体渲染 / 🆓 豆腐块 / 对比表重排）**：①**emoji 单字符也会渲染成豆腐块**——对比表表头「🆓 离线免费版」的 🆓（U+1F193）在部分 Windows Chromium 渲染为残缺方框，而同屏 📋📝🔑🎁 等 emoji 全部正常，说明根因是**单字符字体回退异常而非整套 emoji 字体缺失**（故无需全站替换 emoji；审查员据 Unicode 版本推断 🟢🧭🆕 同险，实测均正常——截图证据优先于版本推断）。根治方式：新增 `.free-mark` / `.free-mark.sm` 纯文字徽章（绿色渐变圆形/圆角方块、白色「免」字，任何系统零依赖），替换全部 4 处 🆓（卡片大图标/对比表表头/FAQ×2）；**举一反三铁律：凡要跨系统稳定显示的标识，优先「CSS 形状+文字」，emoji 只作可缺失的装饰**。②字体：字体栈改 `-apple-system, BlinkMacSystemFont, 'Segoe UI', 'PingFang SC', 'Hiragino Sans GB', 'Microsoft YaHei'`（Segoe UI 提前管拉丁数字、中文按平台回退），body 加 `font-size:15px / text-rendering:optimizeLegibility / -moz-osx-font-smoothing:grayscale`；全站辅助文字 10/11px → 12px（最小正文 12px 业界标准），纯装饰 ▶ 箭头 8/9px 不动。③对比表：`table-layout:fixed + min-width:760px`（首列 th width:118px、其余 5 列均分），字号 13px（`@media max-width:768px` 块内 14px——注意该媒体查询是**手机/平板加码**非桌面），th white-space:nowrap 加粗，单元格 padding 加大；窄屏表格在 .table-wrap 内横滚，文字不再竖排挤压。**P1 教训（审查实测）：fixed 布局 + 首列 `td:white-space:nowrap` 导致长功能名（「数据备份/恢复/自动备份」等）溢出 118px 单元格与第二列叠印——fixed 表格首列宽被永久锁死，长文本只允许换行不允许 nowrap；修复=去掉 td:first-child 的 nowrap（th 保留），Range.getClientRects 几何检测零碰撞。** 验收：双副本字节级 SHA-256 一致、download 基线 0/0、90 项 Playwright 全过、check-interface 6 OK、commit 45d1a224 已推送。生效方式：仅官网静态页 push 即部署，五端零重打包。
...
* ★ 2026-09-22 **验方设置增加「编辑」（同一表单三模式：新增/prefill/编辑）**：需求=对已有验方的药物组成增减、改剂量，名称/简码/功效/主治可改。方案=showAddFormulaForm 加第二参数 editIndex：editing 命中时标题「编辑验方」、4 字段 escapeHtml 预填、composition 逐行 addRow、保存按钮「保存修改」、保存时 `formulas[editIdx]=updated` 原位替换（非 push，其他验方索引不动）；新增 editFormula(index) 薄包装（Number+存在性守卫）；renderFormulaList 在「调用」「删除」之间插「编辑」。**审查发现并修复**：addRow 的剂量预填补 escapeHtml（防恶意备份字符串剂量在编辑时注入，与 08-30 XSS 收口同款）。**核查实锤**：formulaSearch 搜索框未绑任何事件=列表永不过滤，onclick 下标恒为真实数组下标；模态遮罩 z-index 99999 编辑期间无法并行删除=editIdx 不会失效；空 composition 老验方编辑时强制至少 1 味药（符合「方子必须能开药」，保持校验不放宽）。基线：editFormula 双侧同体入 desk Tier B（217）、siteadmin Tier A onlyA（153，无验方功能合法分叉）。验证：新冒烟 smoke-edit-formula.cjs 42/42（预填/药物增减改量/原位更新/兄弟验方不受影响/编辑后调用/取消零副作用/越界防御/删除回归，零 pageerror）+ smoke-add-formula 回归 30/30 + check-interface 6 OK + 4 对基线全绿。铁律：**「新增/prefill/编辑」同表单多模式时，editing 判定必须最严格（非空+有限+≥0+目标存在）且分支优先级最高；编辑保存一律原位替换保索引稳定**。生效：云端网页 push 即生效；云端APP WebView 实载线上 public 即时生效（assets 兜底随下次 APK）；云桌面/离线桌面 index.html 在热更清单（generate-desktop-hotupdate 两渠道均含）可发热包免重装、也可随下次整包；离线APP index.html 本就是热更入口可热更免重装。
...
* ★ 2026-09-23（晚）**一键打包/发布入口审查 + 版本基线漂移门禁（verify-version-baseline.ps1）**：1.0.254 实战踩坑机制化——直接调 `one-click-pack.ps1 -AutoMode 2`（未走入口 bat=无 -AutoCommit）打出的 253 副作用未入库且事后工作区被还原，晚上再打时 HEAD 基线停在 252、dist 现存 253 产物，bump 后只出 253 与旧产物**同名覆盖**，预期 254 消失白跑 10 分钟。新增 [verify-version-baseline.ps1](file:///d:/trae_projects/kyt-zy/tools/verify-version-baseline.ps1)，接入点=one-click-pack.ps1 pack-gate 之后（交互菜单与 AutoMode 都经过）：**形态B** 工作区 package.json version / build.gradle versionCode 与 git HEAD 不一致（上次 bump 未入库）→ FAIL；**形态A**（仅桌面，APP 产物名不含 versionCode 且不依赖 aapt）dist exe 文件名最高版本 > HEAD 基线 → FAIL，文案直接算出"本次只会重新生成 X 同名覆盖，不会得到 X+1"并给两条修复路径（补提交对齐 / 归档旧产物重打）。退出码契约：2=漂移必拦、1=工具自身异常→WARN 放行（工具故障不得阻断打包）、0=通过；保险丝 `ALLOW_VERSION_DRIFT=1`；未跟踪文件不拦（与 source-settled 同口径）；4 单元=本地/云端桌面（package.json+dist）+本地/云端 APP（build.gradle 只检形态B）。**入口 bat 同步加固**：①两入口保存原代码页（`for /f "tokens=2 delims=:" %%c in ('chcp')`）并在统一 `:do_exit` 标签恢复（goto 传 EXIT_CODE 变量，**不能用 call——call 返回后会继续执行主流程**），CI/终端链式调用不再遗留 65001；②一键打包.bat 防 `-AutoCommit` 重复（`echo %* | findstr /i /c:"-AutoCommit"`，PowerShell 重复绑定命名 switch 直接报错）。**铁律：①"必须记得做 X"的人记防呆复发一次就工具化——版本号入库靠 -AutoCommit 自觉不够，必须打包前机器比对 HEAD 与现存产物；②门禁脚本自身异常一律降级 WARN 放行，检测工具不能成为比被防事故更高频的阻断源；③入口/工具类 .bat 必须 ASCII-only+CRLF（Write 工具默认写 LF，落盘后用 Node 转 CRLF；这两个根入口不在 entry-selfheal 的 8 个下游 bat 自愈清单内）；④PowerShell 嵌套调 powershell -Command 时 `$` 变量会被外层吞，AST 校验直接在当前 shell 内联 ParseFile 或落地临时 .ps1**。发布链审查结论（无需改）：publish-release.js 默认强制 compliance-check（L707 `if(!skipCompliance)`，菜单永不传 --skip-compliance）、--confirm 人工确认、auto-publish 默认只检查、--artifact/--target 双维防旧产物误传、复制后 SHA256 逐字节校验——"产物禁止自动上传官网"在工具层是闭合的。
...
- **CORS null**：entitlement/heartbeat corsHeaders 显式放行 Origin 字符串 "null"（file:// 渲染兜底路径）。
- **APP shim（MainActivity.java 注入 JS）**：license 对象补 getMachineId；渲染兜底 machineId 多级链 StorageAdapter→license.getMachineId→activate.getMachineId→status.machineId，取不到明确拒绝。
...
* ★ 2026-09-21 【双页面上下文铁律】云端桌面登录窗与主窗口是**两个独立页面上下文**：login.js（登录窗）里 auth-core login 的 edition-sync / localStorage 写入对主窗口的 `CONFIG` 对象**完全无效**（localStorage 跨窗共享，内存对象不共享）。云端桌面真实主流程 = 登录窗登录 → 主进程 login-success → 主窗口 checkLoginStatus 的 Electron 自动登录分支——此处 `updateUserDisplay()` 前必须以 `electronUser.clinicEdition`（login.js 从服务端登录响应携带、经 IPC 存 currentLoggedInUser 透传）为权威同步 CONFIG.edition / window.EDITION / Permission。历史三连报同根：08-21 离线端同款竞态（__appConfigReady 方案）、09-13 云端只补 refreshVersionTags 版本标签（标签对、按钮错）、09-21 云端自动登录分支补 CONFIG 同步（Commit 349a997b）根治。**深层原因**：热更新流程下 desktop-windows.cjs 从 asar 复制**出厂模板** config.json 到热目录，渲染层同步 XHR 读到的是 cloud_personal 模板；注意 permission.js 的 Permission.init **会**调 get-app-config IPC，主进程返回 `{...defaults,...userDataCfg}`（真配置），但其回调**绕过 edition-lock setter 直写 `CONFIG.__authoritativeEdition` 权威插槽**——慢 IPC 晚 resolve 可反向盖回。0921-3 热包（Seed-2.1-Pro 审查修复）已封死：解析期留存 `window.__permInitPromise`，自动登录 edition-sync 前先 await，登录权威最后写入必胜。**待办评估**：向云端移植离线端 __appConfigReady 全量配置加载（风险：Object.assign 旧快照可能覆盖新登录权威值，须排在 clinicEdition 同步之后）。
* ★ 2026-09-21 【JS 作用域审查铁律·举一反三】①**经典内联 `<script>` 顶层 `let/const` 是全局词法绑定，不成为 window 属性**——本仓 `let currentUser`（index.html 顶部）被误读为 `window.currentUser`，全仓零赋值致守卫/C 步降级两处代码常年死执行（AST+全仓 grep 才实锤）。引用脚本级状态一律用裸变量名，跨页面才用 window 显式挂载（且必须有挂载点）。②**button-manager 在 DCL 时整体替换 enforceStandardEditionButtons**，原函数体仅在 button-manager 加载失败回落或 DCL 前调用存在——改原函数逻辑时必须同步评估替换版 `__enforceStandardEditionInner`（其 `_roleSaysAdmin` 才是线上真正的角色保护：admin/clinic_admin 永不本地降级）。③本地强制函数的机构豁免名单（instDeploy/INST_ED）必须与 permission._normalizeEdition / button-manager 口径对齐（含 institution/institutional 别名）。**铁律：双窗口架构（登录窗+主窗口）下，任何"登录时同步"必须显式检查目标状态在哪个页面上下文；改按钮/权限判定先画全链路：登录窗→IPC→主窗口恢复→updateUserDisplay→Permission 谓词→button-manager 断言。**
...
* ★ 2026-09-23 **后台删账号 ↔ 离线设备吊销联动（账号墓碑，commit 78b74dd2）**：**根因**——后台删除登录用户（`/users?action=delete-user`）此前只删云端用户表+撤销云端 token，而离线登录是【本地密码校验】、登录闸门只按 machineId 裁决激活码，两套裁决互不联通，账号被删后离线桌面/APP 仍可登录。**机制（三层）**：①服务端账号墓碑——`license-core.js` 的 `writeAccountTombstone/getAccountTombstone/clearAccountTombstone`（KV key `account_tombstone:{username小写归一}`），delete-user 删除+撤 token 后写墓碑，**不设 TTL**（到期自动解封=绕过吊销）；真实重新开通（`admin-account.ensureClinicUser` 激活 / `add-clinic_user` 管理员补建）才清墓碑。②entitlement **只读**消费——请求体增 `username`，只读查墓碑，命中即在响应带 `accountState:'ACCOUNT_REVOKED' + accountDeletedAt`（纯 kv.get，不破坏探针 F3 纯只读铁律）。③客户端硬拒——两处闸门权威源（`shared/license/license-manager.js` 主进程 + `shared/auth-core/offline.js` 渲染层/APP）收到 ACCOUNT_REVOKED 即拒绝，**即使设备授权 LICENSED**；双锚点（gate.dat+.license-anchor / APP 用 localStorage `license:accountReject`）写**账号级**拒绝标记，断网也不享宽限，按 username 隔离不影响同机他人；墓碑清除（LICENSED 无 accountState）时清标记恢复。IPC 链 `preload.verifyGate(username)→main→verifyLoginGate`，login.js/index.html 传入当前用户名（APP 无此 IPC 走渲染层真 fetch，墓碑同样生效）。**文案**："该账号已被删除，无法登录。如有疑问请联系客服"。**铁律：①吊销类标记严禁设 TTL，只能靠"真实重新开通"这个带写副作用的动作解除（与第①条安全审查铁律同源）；②账号级拒绝必须按 username 隔离，禁止只按 machineId 否则同机其他账号被连坐；③账号删除/激活码删除/设备封锁是三种独立裁决（account_tombstone / license / device_block），新增删除入口要逐一核对是否需要联动离线吊销**。证据：线上 REST+OAuth 实测闭环（无墓碑 accountState 空→写墓碑→返回 ACCOUNT_REVOKED+删除时间→删墓碑→空）；冒烟主进程闸门 34/34、渲染层登录 17/17、首注 20/20；copy-consistency exit 0；check-interface 6 OK。**坑：OAuth oauth_token 过期（code 10000）用配置里的 refresh_token 向 `https://dash.cloudflare.com/oauth2/token`（grant_type=refresh_token, client_id=`54d11594-84e4-41aa-b438-e81b8fa78ee7`，公客户端无 secret）换新，wrangler CLI 本机崩溃时的可靠替代**。生效：服务端 push 即部署；离线桌面/APP 因改了主进程 preload/main.js **需重新全量打包**（`NO_BUILD_SKIP=1 tools/one-click-pack.ps1 -AutoMode 2`），云端两端不重发。

* ★ 2026-09-23 **账号墓碑阶段2：删诊所缺口 + 手机号别名 + 历史数据回溯（已删用户 zm/13312345678 仍能登录事故）**：**产品铁律（用户拍板）：只有后台「诊所/用户管理」与「激活码管理」中【现存】的账号允许登录；账号一旦删除（含整诊所删除）一律不得登录，多用户绑定/测试机历史/本机测试均不构成放行理由**。**根因二（阶段1只堵了 delete-user 单删路径）**：平台管理员删整诊所走 `/users?clinic=delete`，该路径只删业务键+撤 token+清激活申请，**不写账号墓碑、不解绑设备**；离线闸门只按 machineId 裁决，设备若仍绑在其他有效码上（测试机 7f5e 同时绑 Y6TT+CA73 两个真实客户码）则凭设备级 LICENSED 照登；且**离线本地账号名可能与云端身份不同**（本机 config username=`zm`，云端/激活身份是手机号 13312345678），只写一个墓碑键必有漏网路径。**修复（functions/api/users.js + license/_lib/license-core.js，node --check 过）**：①删诊所分支对被删 users 逐个 `writeAccountTombstone({username,phone,clinicId,deletedBy,reason:'delete-clinic'})`，计数 `tombstonedAccounts` 入审计 extra；②delete-user 调用补传 `phone:target.phone||''`；③`writeAccountTombstone` 增**手机号别名键** `account_tombstone:{phone}`（归一后非空且≠username 才写，值与主墓碑同形、entry.username=phone，写失败不阻断）；重新开通侧无需改：ensureClinicUser 的 username===phone 清一键即双覆盖，add_clinic_user 无 phone 字段、残留别名恰代表"该手机号身份未恢复"语义正确。**历史回溯（wrangler KV 直写，无需部署）**：以全量 `clinic:*:users` 现存身份为白名单，遍历 4 个 `clinic_backup_*` 备份，备份里有/白名单里无的身份补 reason=`delete-clinic-backfill` 墓碑（13800000000、13700000000、13312345678、13900000001 共 4 键），另为本机本地名单独补 `account_tombstone:zm`（clinic_yb93a7z7e8）；实测 entitlement：zm/13312345678 均返回 `accountState=ACCOUNT_REVOKED`（state=NO_LICENSE 与账号吊销双保险）。**即时止血**：经后台会话 `POST /api/license/status?action=unbind` 把测试机 7f5e 从 Y6TT/CA73 两码解绑（客户自有设备绑定全部保留），设备级立即 NO_LICENSE，1.0.253 旧包不升级也登不进。**坑：①PowerShell 5 `Set-Content -Encoding UTF8` 写文件带 BOM → wrangler --path 入库的 JSON 带 EF BB BF 前缀 → Function 端 `kv.get(key,'json')` 解析抛错 → getAccountTombstone 静默按无墓碑返回 null（键明明存在、wrangler get 也读得到，就是裁决不命中！）——写 KV 的 JSON 值一律用 Node `fs.writeFileSync(f, json, 'utf8')` 无 BOM**；②**KV 边缘传播约 1 分钟**：wrangler 直连 API 立即可见 ≠ Pages Function 边缘节点立即可见，验证要间隔重试；③排查删除类漏网必查"所有删除入口"：delete-user / clinic=delete / 激活码删除（license 级另算）三条路径逐一核对墓碑与设备解绑；④Node `execFileSync` 在 Windows 调 npx 必须 `'npx.cmd' + shell:true`（否则 ENOENT/EINVAL），正则批量改调用时别把 `('npx',` 替成 `(NPX),` 造成括号错位；⑤误覆盖/误删仓库根文件后用 `git checkout -- <file>` 恢复，提交前必看 `git status`。生效：回溯 KV 数据立即生效（新旧客户端都受账号吊销约束，新客户端才有 ACCOUNT_REVOKED 硬拒消费）；users.js/license-core.js 改动需 push 由 CF Pages 部署后，未来删诊所/删用户自动带墓碑+手机号别名；客户端账号级硬拒逻辑已在源码（commit 78b74dd2），待 **1.0.254** 新包全量生效（覆盖设备仍绑有效码的场景）。

* ★ 2026-09-23（晚）**账号墓碑阶段3：1.0.254 发布前双代理复审拦下 4 个绕过/锁死洞（热更中和为发布阻断级）**。双路独立复审（两子代理结论收敛，这种模式以后授权链发版照用）发现：**①热更包会整体中和客户端修复（最致命）**——早上 09:18（修复 commit 之前）签发的 `2026.09.23-1` 热更包（desktop/local + app-local）内 auth-core.js/index.html 是无 username 旧码；安卓登录入口整页可热替换，新 APK 第二次启动拉旧热包=墓碑拦截全失效；桌面主窗登出后内嵌登录也走旧热码。热目录不绑 appVersion、全量升级不清除，**修复闸门类渲染层代码后必须同步重签发热更包并 bump 同日序号**（`generate-desktop-hotupdate.cjs -c local -n 2` + `generate-app-hotupdate.cjs -n 2` → 2026.09.23-2），发 APK/exe 前先推热包。**②空 username 清标复活宽限**：主窗自检（offline.js installMainWindowGate）、激活窗关闭复核（activate.js）两处调 verifyLoginGate 不带用户名，旧码 LICENSED 分支 `(!username || …)` 把全机账号拒绝标记清空→同机他人正常联网一次就为被删账号滚动解封。修复：**username 缺失时绝不清除任何账号标记**（只删 map[username] 键），两处补传当前登录用户名（主窗读 `auth:currentUser`；activate.js 经 main.js `bindUserContext(()=>currentLoggedInUser)` 注入）。**③accountReject 单槽→按用户名集合**：`gate.accountReject`/`anchor.accountReject`/APP localStorage 全部改 `{__arMap:1, username:{username,state,at}}`，读入自动迁移旧单条（顶层带 state 标量判定），双锚点合并取同名 at 更新者；多账号先后被删互不覆盖、单账号开通不连坐。**④服务端删/开不对称会锁死合法重开**：墓碑手机号别名双写但清除只清单键（用户重新激活后改名回旧名被永久误锁）→ 墓碑 entry 增 `linkedNames`，clearAccountTombstone 读记录双键同清；补两个删除/恢复入口：clinic=restore 末尾对恢复用户批量清墓碑（否则恢复后全员在线即封）、批量保存用户（POST /users 无 action，processUsersForSave 名单重建）名单外既有用户=被删，必须撤 token+清会话+写 reason=batch-save-delete 墓碑（平台管理员缺名=400 拒绝，禁批量移除；判定只认提交名单里显式带本诊所 clinicId 的用户，防无 clinicId 旧数据摊到所有诊所误伤）；新出现用户名=重新开通清墓碑。**顺带实锤修复导入旧洞**：import_users 分支 `processUsersForSave(新名单, 存量)` 只返回新用户却整体 put=整诊所存量被覆盖删除——改为 `[...existingClinicUsers, ...importedSaved]` 合并写回。**铁律：a) 授权链发版必查"修复代码是否在当前热更包里"——热更优先级高于整包，旧热码可中和新 APK/exe，发版清单=源码+热包+整包三位一体；b) 吊销标记清除只能由"具体用户名+在线 LICENSED"触发，无用户名上下文宁可不清；c) 删除口径盘点穷举所有写 users 数组的路径（delete-user / clinic=delete / clinic=restore（反向=清）/ 批量保存 / 导入 / 激活开通（反向=清）），每条都要决定墓碑写或清；d) 任何"处理名单后整体 put 数组"的接口必须先 diff 存量，合并或显式处理消失项**。验证：主进程闸门冒烟扩到 37/37（新增 N34 他人在线不清 A 标记+A 断网拒/B 宽限、N35 空 username 不清标、N36 双账号集合+单开通不连坐），渲染层 17/17；sync-all/auth-core 全副本同步、check-interface 6 OK。生效：服务端+热包 push 即部署；整包 1.0.254（exe+APK）携带主进程新码。
...
## 22. 离线免费版（free 授权档，2026-09-21 阶段0已上线并真机闭环验证）

> 方案文档：`.trae/documents/离线免费版_plan.md`（已批准）。产品决策：**同一安装包 + 授权分档**，激活窗加【🆓 免费使用】绿色入口；**不新增 edition key**——free 复用 edition=personal 标准版形态，差异全部由 `license.type=free` 表达；开方不限量（maxPrescriptions=0）、永久有效（expiresAt=2099-12-31）；免费授权走服务端签名下发（联网领取一次、机器绑定、之后永久离线）；付费墙=备份/恢复/自动备份、Excel/CSV 导入导出+统计导出、拍照录像、无水印打印（打印免费但带水印）；输付费激活码原地升级、数据零迁移。commit 31e4bc1a（+基线重冻结 19756d80）。

### 22.1 授权链坐标（后续阶段在此扩展）
* **shared 权威源**：`license-manager.js` LICENSE_TYPE_CONFIG 首位 `free:{maxPrescriptions:0,features:[]}`；`applyEditionBindingToConfig` 放行条件加 `&& type!=='free'`（free 落标准版分支校正 edition=personal）；`feature-guard.js` 新增三功能位 `data-export` / `media-capture` / `print-clean`（FEATURES+FEATURE_NAMES_CN+checkFeature free 升级文案+checkDataExport/checkMediaCapture/checkPrintClean）。
* **服务端**：`POST /api/license/claim-free`（`functions/api/license/claim-free.js`，常量 FREE_LICENSE_EXPIRES_AT/FREE_SERIAL_CODE='FREE'/KV 前缀 `free:device:{machineId}`；schema-guard 验机器码、IP 频控 20/小时、手机号选填 `/^1[3-9]\d{9}$/`、**每机一份可重领身份首次锁定**；调 buildLicenseData 自动附 v7 Ed25519，响应与 validate 同构）。license-core：free 在 LICENSE_TYPE_CONFIG，**同时被排除出 PAID_LICENSE_TYPES 与新导出的 CODE_ISSUABLE_TYPES**（generate/batch 后台开码永不生成 free 激活码——防免费档泄漏进付费开码通道）。
* **桌面端**：activate.js `claimFreeOnline(machineId,phone)`（12s AbortController+15s race 超时；phone 非空才建手机号账户空密码默认 admin，不填→登录窗走注册向导）；main.js IPC `license:claim-free`（L1447）；preload `activate.claimFree`；activate-window.html 绿色按钮 id=`claimFreeBtn`（startTrialBtn 后，试用到期态保持可点=核心出路；licensed 后禁用显示「本机已开通免费版」）；index.html `getEditionTag` 加 free 分支显示【离线免费版】+ `loadLicenseTypeForTag()` 启动拉取重刷（**desk 跨版本基线已人工确认合理单侧分叉**：云端登录制无此函数）。

### 22.2 ★ Electron 界面全自动 E2E 方法论（Playwright 官方旁路，2026-09-21 实测 14/14 PASS）
* 桌面 package.json devDeps 自带 playwright：`const {_electron}=require('playwright')` + `electron.launch({executablePath: node_modules/electron/dist/electron.exe, args:['.'], env:{...process.env, BNZC_E2E:'1', BNZC_E2E_DATA:'<隔离目录>'}})`。**必须双条件开旁路**：env `BNZC_E2E=1` + 在 `node_modules/electron/dist/` 写 `e2e-enabled.marker`（main.js blockRemoteDebugging L77-103；marker 跑完即删，生产包永不携带）；BNZC_E2E_DATA 同时把 userData 重定向，绝不污染真实授权目录。
* **原生弹窗（dialog.showMessageBoxSync 阻塞主进程）Playwright web API 点不了**：用 `await app.evaluate(({dialog,ipcMain})=>{ dialog.showMessageBoxSync=()=>0; dialog.showMessageBox=async()=>({response:0}); ipcMain.removeHandler('license:restart'); ipcMain.handle('license:restart',()=>{globalThis.__seen=true;return {success:true}}); })` 主进程注入自动确定 + 把重启 IPC 改为打标不重启（保活做断言），轮询 `app.evaluate(()=>!!globalThis.__seen)` 等领取完成。
* **登录密码框防自动填充坑**：desktop-windows.cjs L364-375 dom-ready 把所有 password 框改 type=text+readonly，readonly 在 **focus 事件**才移除。Playwright `.fill()` 直接填必超时——先 `.click('#loginPassword')` 聚焦 200ms 再 fill。
* 窗口发现轮询 `app.windows()` 按 url().includes('activate-window.html'/'/index.html') 找新窗口（激活窗/主窗都是独立 BrowserWindow）。
* 热更坑：隔离实例会自动下载旧热包，重启即遮蔽本地新 index.html——验证标签前删掉隔离目录的 `hot-update/`（activate-window.html 不属热更覆盖范围）。

### 22.3 阶段0验证与生效方式
* 生产端对端：真端点签发 free license（v7 Ed25519，kid=v7-001），客户端 shared 权威 verifySignature 直过；重领幂等/非法机器码/非法手机号均拦；生产链 11 项 PASS。
* 真机 GUI E2E 14/14 PASS：非法手机号界面拦截、无手机号领取落 license.dat(1310B)+trial 清除、重启后 getStatus licensed/free/max=0/features=[]、激活按钮已开通禁用态、带手机号领取、手机号/admin 登录后主界面右上角【离线免费版】+ edition=personal。
* 生效：服务端 claim-free push 即生效（Pages 自动部署）；云端网页/云桌面/云端APP 零改动；**离线桌面主进程+激活窗改动必须发新整包**（热更不覆盖 electron/ 目录），阶段3 统一打包；离线APP JS 副本已随 sync 就位，Java 侧阶段2补齐后随 APK 发布。
* **后续**：阶段1=桌面功能墙（requireFeature 包装器接 `license.checkFeature` IPC，备份3触发点/6 导出导入入口/统计导出/拍照录像渲染+desktop-fs-ipc.cjs 主进程双拦 fail-closed）+ printPrescription JS 动态注入水印（禁改 HTML/CSS，先出样张确认）；阶段2=Java LicenseManager free 分支（L3408 类型映射加 free）+ **第一步必须真机实测 Java 验签是否支持 v7（R3 未核实，不兼容则按客户端版本下发其支持的签名格式）**；阶段3=官网免费卡/对比表/FAQ（商业文案用户确认前不设置付费项目）+整包打包+KNOWLEDGE 收尾。

### 22.4 阶段1已完成（2026-09-21，水印 9/9 + 功能墙 16/16 E2E 全过，待 commit）
* **★ 付费墙唯一判定铁律：licenseType==='free' 才墙**。trial（无 license/验签失败落 trial）= 评估期全功能放行；历史付费 license 内嵌 features 不含 data-export/media-capture/print-clean 新位，**不能**用 checkFeature().allowed 直接裁决（会误伤 personal/pro）。实现：shared/feature-guard.js 新增并导出 `isFreeEdition()`（getLicenseType()==='free'，异常 false）；渲染包装器 `window.requireFeature(feature,{silent})`（index.html loadLicenseTypeForTag 后）调 license:check-feature，仅 `r.licenseType==='free' && !r.allowed` 拒绝（alert 文案 + confirm 激活窗 electronAPI.activate.show），IPC 异常 fail-closed；非 Electron 环境无此 API → 放行（护 APP/网页）。
* **主进程执行点双拦（shared/desktop-fs-ipc.cjs）**：工厂内 lazy `require('./feature-guard')`（云桌面 electron/ 同有副本但永不签发 free，零影响；require 失败不墙），`freeWallReason(cn)` 返回拒绝串/null。门控通道：save-backup-file/save-auto-backup/list-backup-files/read-backup-file/open-backup-picker（backup）；**媒体通道按文件名收窄**——save-prescription-image 仅当文件名含 `_photo_` 拒（`_prescription.png` 处方笺打印自动存档必须放行，打印免费），save-video-file 仅当含 `_video.` 拒（拖入外部视频不拦）。拒绝返回 `{success:false,error,freeDenied:true}`。
* **渲染门控落点（离线权威源 index.html）**：backup=exportData/importData（改 async，收口 list/picker 两路径）/dailyAutoBackup/window.__bgAutoBackup（后两者 silent 静默短路，plan R6）；**clearAllData 不门控**（计划矩阵只有备份/恢复/自动备份，清空是用户自救手段）；data-export=exportMedicines/exportMedicinesToExcel/exportMedicinesToCSV/exportPrescriptionsToExcel/exportPrescriptionsToCSV/importMedicines（拒绝时清 input.value）/exportStats 共 7 个（纯 Blob 下载，渲染门即执行点，无主进程通道）；media-capture=electron/video-recorder.js 注入的 #photoCaptureBtn/#videoRecBtn 两个 onclick async 包 requireFeature（仅此离线副本，非 shared，云副本勿动；主进程 IPC 兜底）。
* **水印终稿（用户确认方案B）**：`免费版` 楷体加粗 42px、rotate(-25 140 105)、fill-opacity 0.08、SVG 280×210 tile repeat；SVG 串 `encodeURIComponent` 后必须 `.replace(/'/g,'%27')`（单引号不编码会提前闭合 `url('')`，实测坑）；overlay div absolute inset:0 z-index:10 pointer-events:none + print-color-adjust:exact；.prescription-paper 加 position:relative（纵横两版共用模板自动覆盖）；isFreePrint 缓存 window.__licenseType，false 时 await getStatus 复核，**查询失败 fail-open**（打印免费只去水印不拦打印）；水印只进送印 HTML 三通道（AndroidNative/electronAPI.printPrescription/window.open），屏幕 #prescriptionPaper 不显示。
* **E2E 续坑（补 §22.2）**：①contextBridge 的 electronAPI 只读，拦截送印 HTML 要走第一分支注入 `window.AndroidNative.printHtml`；②waitForFunction 不能查 window.currentUser（顶层 let 不挂 window），改 await electronAPI.getCurrentUser()；③全新首启是 auth-core `showLocalRegisterModal` 一页式本地注册（非 wizard：填 #localRegClinicName/#localRegAdminName/#localRegPhone/#localRegPassword(2) → #localRegSubmitBtn → #localRegLaterBtn → 页面 reload → 手机号+密码登录）；④confirmSync 按钮顺序 `['取消','确定']` 确定=1，E2E 补丁 dialog.showMessageBoxSync=()=>1 才是模拟"确定"（=0 是取消）；⑤Playwright w.url() 返回 Promise 必须 await；⑥隔离实例 hot-update 每次启动前删（旧热包遮蔽新 electron/ 与 index.html）；⑦真写媒体目录（D:\惠康中医媒体\downloads\YYYY-MM）的探针文件用完按返回 filePath unlink 回收；自动备份写隔离 userData/backups 不污染。
* **阶段1验证**：check-interface 6/6；sync-all 全副本就位（feature-guard 5 副本+desktop-fs-ipc 3 副本+index-app/APP 资产双写）；desk 跨版本基线重冻结（8 个门控函数+1 收敛，Tier B=218/Tier C=55 全绿）；水印 E2E 9/9（free 纵横有水印、trial 无、A5 orientation、relative 基线，真送印 HTML 渲染 PNG 肉眼复核）；功能墙 E2E 16/16（free 三渲染门控全拒、5 备份 IPC 全 freeDenied、相机照片/录像拒、处方存档/外部视频放行、自动备份零落盘、手动备份唤起激活窗；trial 三门控放行+自动备份真实落盘）。临时脚本与隔离数据已清，e2e marker 已删。
* **待办/生效**：付费升级路径（free→输码 personal）需真付费码，留阶段3打包前真机验证（静态语义：installLicense 覆盖 license.dat 后 isFreeEdition 即 false 全门开）；高风险授权链变更 commit/push 前过 Seed-2.1-Pro 独立审查 + 安全二查；electron/video-recorder.js 非 shared，已装机用户随 local 热包送达（阶段3 generate-desktop-hotupdate 打包），desktop-fs-ipc 主进程改动必须发新整包；index.html/index-app 渲染改动可热更。

### 22.5 阶段2 v296-v299 真机实测（2026-09-22，免费版落地实锤）
* **★★ 旧热包遮蔽新整包（本轮最大坑，发布铁律）**：`HotUpdateManager.resolveEntry` 启动复验只验签+哈希，**不比对 APK versionCode**（设计注释：versionCode 只升不降，热包永远适用）。后果：APK 覆盖安装后 filesDir/hot-update/current 旧热包继续生效，**白名单 14 文件（index.html/auth-core.js 等）整包里的新版全部不加载**。v299 真机三症状（授权区显示"已激活剩余26765天"+翻出历史付费码 BNZC-...、语音墙失效、打印水印回退）全因 09-18 旧热包遮蔽 v297-v299 整包。**铁律：凡 APK 改动触达热更白名单文件，发版必须同步 `node tools/generate-app-hotupdate.cjs` 重签热包并 push 部署**（生成器源=assets/public 当前内容，自动 LF 规范化；minAppCode 默认继承勿乱抬）；只发整包=旧热包原样遮蔽。客户端：启动 2s 静默拉取→原子 swap→**再下一次启动生效**（用户须联网开一次、退出重进）。排障首查：真机异常先怀疑热更目录而非整包内容；E2E 隔离实例同样每次删 hot-update/。
* **Java validateLicense free 返回形态**：`{valid:true,type:'licensed',licenseType:'free',remainingDays≈26765,features:[],maxPrescriptions:0}`（2099 到期）。JS 读状态必须分清 `status.type`（licensed/trial/expired）与 `status.licenseType`（free/personal/...）；offline.js updateLicenseStatusText 历史写法 `licenseType=status.licenseType||status.type` 取到 'free' 后三个分支全不匹配落 else"未知类型"→误显已激活+天数。free 档授权区文案="🆓 永久免费版（开方不限量）"，**不显示剩余天数、不调 renderActivationCode/loadInviteInfo**。
* **历史付费码泄漏**：renderActivationCode 的 getLicenseCode 来源4（机器码联网 admin-status 找回）会把该机历史机构版码翻给免费用户——开头必须 free 早退（getStatus type=licensed/licenseType=free return）。
* **Android 打印水印**：WebView 系统打印（createPrintDocumentAdapter→存 PDF）丢弃 CSS `background-image`（SVG data-URI、print-color-adjust 均无效）；解法=送印 HTML 内放**真实 DOM span 文字斜铺**（14行×12列绝对定位+外层 rotate(-25deg) overflow:hidden），桌面 Electron 同源码兼容。
* **语音墙（v298 第 5 类）**：feature-guard VOICE_INPUT='voice-input'；只门控整方语音解析面板按钮（index.html #voiceEntryMainBtn 及 APP 注入的 injectSmartTextEntry 按钮，async 首行 `if(!await window.requireFeature('voice-input'))return;`）；字段旁小麦克风 Web Speech API 在 APP WebView 本就不渲染；药名框"黄芪15克"键盘增强不封。Java 零改动（features=[] 天然拒）。desk 跨版本基线已人工确认：injectSmartTextEntry 语音墙=离线专属合理分叉（云端标题明示"语音录入（免费）"不加墙）。
* **免费版 × 已激活用户关系（v299 产品规则，三层防护）**：①已是 free→幂等成功 `{success:true,alreadyFree:true}` 不重启；②付费有效期内→**执行点硬拦**（Java claimFreeLicense + 桌面 activate.js claimFreeOnline 在写 license 前 readLicense+verifySignature：未过期返回 `{success:false,errorCode:'ALREADY_PAID'}`；检查异常 fail-open 不误伤新用户）；③付费已过期→允许领取但前端 confirm 强提示二次确认（5 功能收窄+原码续费输码即恢复）；④trial/未激活→直接领。前端两个入口都加四态前置：shared/auth-core/offline.js 管理员向导（#adminClaimFreeBtn，APP+桌面主界面）与 desktop/electron/activate-window.html（#claimFreeBtn 独立激活窗，用 confirmSync，非 shared 无同步链）。服务端 claim-free 不查付费（有意设计：客户端最清楚本地授权状态，付费绑定记录始终保留可恢复）。
* **激活链铁律重申**：syncCreateActivationUser 永不覆盖已注册密码（'admin' 占位不 UPSERT）；accountExisted 回传控制弹窗三分支；doctorName 仅空时补防被手机号覆盖。
* **v300 独立代码审查（Seed-2.1-Pro 流程，双 sub-agent 交叉验证 6 项候选）**：2 项确认已修、4 项证伪排除。修复①建号回读取证铁律——Java `syncCreateActivationUser`/桌面 `installLicense` 内联建号均**内部吞异常**，`accountCreated` 严禁用"填了手机号"冒充，必须建号后回读 config.users 确认（Java `ensureFreePhoneAccount` 返回 [落盘,已存在]；桌面 license-manager 新增导出 `localUserExists`（主目录+userData fallback 两处都查）/`ensureLocalActivationUser`（签名写盘+回读），activate.js install 前记 existed、install 后回读 exists，三态 accountCreated/accountExisted 回传，activate-window 弹窗补第三态"用注册密码登录"）。修复②alreadyFree 短路带新手机号时**幂等补建账号**（老免费用户补绑路径；建号函数不覆盖已有密码所以安全），前端 alreadyFree 弹窗按 accountCreated/accountExisted 给补建提示。证伪项：getDefaultMaxPrescriptions free 落默认分支（PERSONAL=0=不限量+服务端签发 max=0）；畸形 expiresAt fail-open（验签内容不可伪造且 validateLicense 已判 invalid=无有效权益可降级，属有意纵深权衡）；桌面 verifySignature 导出/type:'expired' 形态/async 化调用点/文件名收窄双端一致性均核实无误。**教训：审查报告必须派 2 个互不通气子代理各自读码取证，分歧项（major vs minor）取证据链更完整一方；修复后 Java 改动必须重跑整包构建（方法名被 R8 混淆，验包用中文字符串而非方法名）**。
### 22.6 阶段3已完成（2026-09-22，官网免费页 + 桌面整包 1.0.251 + 密码重置定论）
* **官网免费版商业页（双副本字节级一致）**：`public/download.html` ↔ `site-official/download.html` 从历史分叉手工收敛为**完全同 SHA-256**（3B2B6B…830C），download 三层基线重冻结 0/0。结构=下载 Tab 顶部免费卡（￥0 永久/三按钮跳桌面卡·APP 卡·对比表）+ 安装指引 3 处补"也可点【免费使用】领永久免费版" + 对比表扩为 6 列（首列🆓离线免费版：￥0永久/开方不限量/备份·Excel·拍照·语音均"❌ 标准版功能"/打印带水印/仅领取时联网一次）+ FAQ 新增 4 条（免费版vs试用区别、付费升级不丢数据、换机重领、忘记密码）。商业文案经用户批准按草稿直接发布。
* **响应式实测（Playwright 本地 chromium，python http.server 8931 承载，file:// 被安全策略拦）**：375px 档 `documentElement.scrollWidth=375`（下载/对比两 Tab 均是），1280 档零 pageerror。对比表 483px 宽是**设计内行为**——由 `.table-wrap` 容器横滑收纳不撑页面；顶部 tab 按钮条同理可横滑。曾有浏览器代理在加载未稳定时报 538 溢出，**以 Playwright load+600ms 后的确定性测量为准**；对照 HEAD 旧页同值，非本次引入。
* **★★ E2E 时机铁律（本轮排查结论）**：成品 win-unpacked exe 在 build.bat [7.9] 已写 fuse（RunAsNode/Inspect/NODE_OPTIONS=off）→ Playwright 1.62 `_electron.launch` 必超时（它先等 `--inspect=0` 的 stderr 行 `^Debugger listening on ws://`，fuse 关闭后此行永不出现；浏览器层 `DevTools listening on` 正常也没用）。**对最终 fused exe 跑 run-e2e 恒红，不是回归**；登录链回归只认流水线 [7.8] pre-fuse 的 E1-E3（pack 日志 `.build-cache/logs/pack-*.log` 查 "E2E PASS pre-fuse"）。补跑 E4-E6 的合法姿势：临时把 fused 主 exe 改名 → run-e2e 自动降级 dev electron + `_backup_asar/real_app.asar`（打包时刷新的 pre-fuse 同款字节，07:16）→ --only E4/E5/E6 → 恢复改名（本轮 3/3 全绿：哈希手机号登录/出厂 admin 封锁/持久注册入口）。注意 verify-fs-domain/verify-windows-domain 的被测目标是 **dev electron 跑源码**（非打包产物），绿只证明源码；marker 双条件门对 dev 路径写 `node_modules/electron/dist/`。
* **免费用户密码重置=零新开发**：桌面登录页与 APP 均已有"忘记密码？"（桌面 index.html L3355-3399 注入 → 账号确认 → 密码重置 admin → electronAPI.renameUser 回写 config.json；APP Java 桥 case "renameUser"）。免费手机号账号/admin 账号通用；**不触碰 license.dat/授权状态/处方数据**。仅 FAQ 明示：重置后用原账号 + 密码 admin 登录。
* **桌面整包 1.0.251**：`one-click-pack.ps1 -AutoMode 2d` exit 0；产物 `app_project/db-offline/desktop/dist/惠康中医-本地 Setup 1.0.251.exe`（75.5MB）与 portable `惠康中医-本地 1.0.251.exe`（75.3MB），pre-fuse E2E E1-E3 全绿、fuse check/签名/smoke 全过；版本号副作用 package.json 1.0.250→251 随提交走。**v300 主进程账号链（localUserExists/ensureLocalActivationUser/付费墙 desktop-fs-ipc）只随整包生效；当前 local 热包仍 2026.09.18-1，渲染层若另推需跑 generate-desktop-hotupdate**。E2E 日志里 `'wmic' 不是命令` 是 Win11 去 wmic 的无害噪音；尾部偶发 TRAE Sandbox Error 但任务 exit 0，以产物+pre-fuse 日志为准。
* **生效方式**：官网两文件 push 后 Cloudflare Pages 自动部署（线上复验 `https://tcm-prescription-system.pages.dev/download.html`）；桌面 1.0.251 exe 需分发给用户安装（热更不覆盖 electron/）；离线 APP 沿 v300 APK（versionCode 300）无需重打；云系零改动。

### 22.7 注册弹窗免费版说明（2026-09-22，纯文案授权链改动的分发边界）
* **注册开通弹窗权威源**：`shared/auth-core/offline.js` 的 `showLocalRegisterModal`（信息框 ≈L3932 + 成功页 ≈L3970），sync-auth-core.ps1 分发离线 3 副本（desktop 根 + desktop/electron + APP assets）；云端 8 副本用 cloud.js 无此弹窗，不会污染。按钮路由：成功页"立即激活正式版"桌面走 `electronAPI.activate.show()`（activate-window.html `#claimFreeBtn`），APP 走 `openAdminActivate`（向导 `#adminClaimFreeBtn`）——文案指引用双端通用版本名【🆓 永久免费版】，桌面按钮"免费使用"/APP"永久免费版"措辞不同，勿只写一端按钮名。
* **★★ 纯文案也受分发边界约束（本轮实证）**：桌面 `electron/login.html` 属 **asar 域永不热更**（hot-update-core/update-manager 多处注释明示），登录窗加载的是 asar 内 `electron/auth-core.js`——**登录窗注册弹窗的文案/逻辑改动只能随桌面整包生效，desktop-local 热包救不了登录窗**（热包 auth-core.js 只服务主窗 index 上下文）。APP WebView 注册弹窗在 index 页面上下文，app-local 热包可达。判断法：改动点在登录窗渲染就排整包，在主窗/APP 页面可热更。
* **热包序号防呆**：`generate-app-hotupdate.cjs`/`generate-desktop-hotupdate.cjs` 的 `-n` **不自动继承既有序号，默认 1**（hotVersion=YYYY.MM.DD-N 按运行日期拼）；漏传 -n 会签出同日 -1，低于线上版本客户端永不更新。发版前必查两 version.json 当前 hotVersion（app-local / desktop/local），显式传下一个序号（本轮 app 2026.09.22-3→-4、desktop local 2026.09.18-1→2026.09.22-1）。`--help` 不被识别会被当成无参执行直接签包（本地误签未 push 不部署，重签正确序号覆盖即可）。minAppCode 不传则自动继承（本轮 288）。
* 文案类 innerHTML 改动过授权链审查：静态字面量无插值=无新 sink；双子代理交叉+安全审查结论一致后，仍走完整 sync/热包/整包链。

### 22.8 Cloudflare 免费套餐 KV 配额治理（2026-09-22，90% 告警实证）
* **配额底数（Free 计划，账户级、UTC 00:00 重置）**：KV 10 万读 + **1000 写** + 1000 删/list 每天；Workers/Pages Functions 请求 **10 万/天**（Error 1027）；升级 Workers Paid $5/月 → KV 1000 万读+100 万写/月、请求 1000 万/月。**1000 写是最先爆的墙**。
* **事故原型**：telemetry/heartbeat 每次客户端启动固定 4 次 KV（ratelimit get+put、tl_dev get+put）= **2 写/启动，500 次启动/天即打爆**；免费版上线 + 多轮真机/E2E（E2E 不跳过心跳，pre-fuse 每轮都真实上报）叠加触发 90% 告警。
* **服务端降载（只改 functions/api/telemetry/heartbeat.js，push 即生效、四端零改动）**：①`caches.default` 边缘缓存按 `ed:midHash:UTC日期` 短路（缓存 Response 只存常量 '1' 且不出端，match 命中仍计 Workers 请求数但 0 KV）；②KV 记录 `last` 已是今天 → 不写（days 从"每日启动次数"变 0/1 日活标志，funnel 只用 first/last 不读 days，无失真）；③限流计数器纯 1/5 抽样写、去掉整点 current===0 锚点（并发突发时锚点会 N 次重复写）；④tl_dev 记录加 expirationTtl=35 天防随机 mid 垃圾键永驻。降载后每设备/天 ≈1.2 写、重复启动趋近 0，**容量 ≈ 800 日活设备/天**；10 万读墙 ≈ 数万设备（边缘同 colo 重复启动 0 读）。
* **残余风险（基线既有，本次未消除）**：随机 mid 刷写每伪设备仍 1 写/天，分布式 IP 各 1 请求可秒爆写配额，且 KV 是**全产品单命名空间共享**，写爆会拖死 claim-free/validate 等授权写路径。缓解：Cloudflare 控制台 Security→WAF→Rate limiting rules 免费计划可配 1 条规则（path 含 /api/telemetry/heartbeat，1 分钟 >60 次/IP → block 1 小时，HTTP 层拦截不进 Worker 不耗 KV；IPv6/64 与代理池绕过属残余）。长期二选一：$5 升级（推荐，写量翻 33 倍）或客户端降频（免费版心跳改每 7 天一次，需四端改+重发）。**E2E 心跳噪声**：BNZC_E2E 旁路目前不跳过 telemetry 上报，测试密集日会虚耗配额，后续可在 main.js 心跳处加 `__BNZC_E2E_BYPASS` 短路（须整包）。
...
### 22.11 取消离线 admin/admin、首次启动强制注册（2026-09-22）
* **结论：离线版 admin/admin 无存在意义**。出厂共享账号导致：①客户拿到即用但账号无身份归属（注册前置后激活/客服有手机号可溯）；②幽灵 admin 长期制造兼容分支；③官网大量「默认密码 admin」误导。**云端流程不动**（云端账号来自服务端，claim 审核初始密码 admin 是服务端行为，shared/auth-core/cloud.js 未改）。
* **改动清单**：①两份出厂 config.json（db-offline/desktop、app assets）`"users": []`；②shared/user-store.js：`getDefaultUsers()` 无 CONFIG.users → `[]`、`get()` catch → `[]`（FALLBACK_ADMIN/isBuiltinDefaultAdmin 保留，仅作列表隐藏幽灵 admin 的判定谓词）；③desktop/electron/login.js：零用户提示注册+openLocalRegister、密码框永不预填、旧三步向导不再自弹、clinicSetupHint 补 onclick（「点击此处」死文案修复）；④desktop/index.html：handleLogin 零账户守卫；handleForgotPassword 改两次 prompt **直接设新密码**（≥8位含字母数字+确认，renameUser 明文回写 config.json）；修改密码原密码错误分支改 alert 引导。
* **零等待注册触发**：offline.js 尾部启动接线加 `fireRegistrationPrompt()`，DOMContentLoaded 立即 `maybePromptRegistration()`（不可跳过：无关闭钮、Esc 无效；成功页才显「立即激活/稍后试用」），与原 2 秒延迟检测幂等（showLocalRegisterModal 有 ID 守卫，JS 单线程 append 不可并发，双弹不成立——审查复核结论）。
* **★ A1 三态安全门（APP 忘记密码，替代 09-11 初版有缺陷的核验）**：初版「读激活记录 code，有码才验」实际 fail-open（桥 reject/success:false/空记录/旧无码全放行）。重构：以 `global.__isDeviceLicensed`（status valid+type≠trial，含 license:code 多源回退）判激活态——**激活机：记录读取失败/无码一律 fail-closed 提示客服 hktzy1688，码核验（去横杠大写归一）通过才放行；试用/免费机：直接设新密码**。关键教训：安全门要以「设备状态」而非「敏感数据能否读到」为前提，否则读失败路径全变绕过。
* **★ A2 config 损坏备份回填**：main.js get-app-config 外层 catch（readJson 解析失败路径）加 `loadUserAccountBackup()` 回填，防老客户 config 损坏+localStorage 空被强制重注册；**backupUserAccounts 截断保护**（新列表账号数少于旧备份不覆盖）。
* **★ A3 Java 密码哈希（需重装 APK 生效）**：LicenseManager 加 `hashPasswordSha`（SHA-256 固定盐 `bnzc_prescription_salt_v1`，与 offline.js/main.js 严格同盐同格式，哈希不可用回退明文），6 个密码写点（registerLocalUser×2/renameUserBridge/syncCreateActivationUser×3）全部统一；历史明文账号 verifyPassword 永久兼容。
* **★ 同步归属救场（22.10 铁律第二次生效）**：A2 的截断保护初改误落 electron/license-manager.js 副本，意识到后重落到 shared/license/license-manager.js → sync-all 5 副本。main.js/login.js 经验证不在任何同步组=真权威，直接改。
* **官网 download.html**：离线相关 admin/admin 文案全改「注册即用」（快速步骤/两份 credential-box/安装登录两处/首次指引/试用期/对比表行/FAQ 试用与忘记密码/购买引导共 13 处）；云端/管理员激活语境保留。双副本（public ↔ site-official）改一覆一，diff pair 零差异；Playwright 双档 0 pageerror。
...
* **遗留（P1 候选，不阻塞）**：diagnose 端点长期应并入 platform_admin Bearer（与 `?check=` 对齐，顺带 key 改头传递+常量时间比较+频控）；客服密钥文件可加 DPAPI/icacls 收紧 ACL。~~客服手册重写~~ → **已关闭，见 §31（P1-7，2026-09-25）**。

## 25. P1 A 批：签发/打包门禁硬化 + 副本纳管（2026-09-24，纯工具/门禁，五端零重打包）

* **①热包序号自动化（两生成器 generate-desktop/app-hotupdate.cjs）**：不传 `-n` 自动读当日已发 version.json +1、跨自然日重置 1；显式 `-n` **不大于当日已签最大序号（同号或回退）一律 exit 1**——双审实证客户端只判 `hotVersion 相等 || signedAt 新旧`（HotUpdateManager.java / hot-update-core.cjs 不做序号大小比较），低序号新签包 signedAt 更新会被已装高序号客户端当"更新"**降级安装**；回灌/同号重签必须显式 `--force`（带告警）；系统日期早于已签日期（时钟回拨）无条件拒签，`--force` 也绕不过；version.json 损坏 fail-closed。**铁律：`-o` 仅隔离测试用，正式发布必须不带 -o 直写 public/，严禁 -o 产物手工落盘（绕过全部序号保护）**。序号决策位于读私钥/哈希/清目录/写盘之前。
* **②打包签名硬失败（sign-exe.ps1 + build.bat×2 + build-app.bat×2）**：sign-exe 新增 `-Strict`（证书材料缺失由 rc=2 跳过改 exit 1）与 `-SelfCheckPath`（签前比对 pfx Thumbprint 与 self-check.js L30 `EXPECTED_EXE_SIGNER_THUMBPRINT`，不符 exit 1 绝不动 exe；正则锚 `^const ...;` 且全文件唯一命中）；四份打包脚本全部接线：EXE 主程序/安装包签名非 0 即硬失败+obfuscate restore，安装包 glob 零命中（for 体不执行 SIGN_FAIL 假成功）也硬失败；APK 侧 apksigner 发现链五源全败改 `goto build_fail`。**坑：self-check.js 是 UTF-8 无 BOM 含中文，PS5.1 Get-Content -Raw 默认按 GBK 读会错位吞 LF，必须 `-Encoding UTF8`**；sign-exe.ps1 本体保持纯 ASCII 无 BOM。
* **③云桌面 E2E 守卫**：cloud_desktop/electron/main.js `reportDesktopIntegrity()` 在 `!app.isPackaged` 后补 `if (process.env.BNZC_E2E === '1') return;`，对齐离线版（2026-09-23 事故：签名前 win-unpacked 自判 tampered state=2，用例活到 ready+25s 上报**构建机自身 machineId** 反遭封锁）。生产无此 env 不影响上报；本地 asar 哈希强阻断与授权链独立于此。
* **④CI 修复**：code-quality.yml 旧断言引用已删除的 desktop_geren、且 title 期望过时（任何 PR 必红）；改为离线桌面「惠康中医诊所管理系统 V1.0.0」、public「惠康中医-云端标准版」。
* **⑤副本纳管**：copy-consistency 由 16 组 77 副本扩到 **19 组 86 副本**：cloud-api.js（权威 shared/，7 副本：public×2、site-admin、cloud_desktop、云/离 APP assets、鸿蒙 rawfile；APP 必需的 `_cloudReachable` 防御初始化在权威源内全端同体）；云系 electron video-recorder.js（权威 public/electron，cloud_desktop/electron 1 副本，本次物理收敛纯落后 29 行，收敛后哈希 1eb77b2fda11 与 cv 锚一致）；浏览器版 public/video-recorder.js→site-admin。sync-all 拆 Group 8/8b、新增 Group 20a/20b，Sync-Group 新增 `-SourceDir`（默认空回退 shared/，仅两个字面量调用点使用）。**video-recorder 拓扑铁律：离线桌面 electron 副本 5ca998497c4b 是含 media-capture 付费墙的真分叉，永不入云系组、禁止拉平；云 APP assets 无 electron/video-recorder.js（inject 版接管）**。
* **流程**：双独立审查（功能+安全互不通气）→ 按收敛意见修复（低序号回退拒签/指纹唯一锚定+UTF8 读取/非 0 即败/零产物检测/`%VAR%` 复合块展开失真改静态文案/-o 红线注释）→ 实测三态+负向全过。安全侧遗留（后续加固，不阻断）：旧 manifest 可读但未验内置 Ed25519 签名（需 main 提交权，同信任域）；清目录后写 manifest 前崩溃可致序号重置（可 git restore）。
* **生效方式**：云端网页零影响；云桌面/离线桌面/云 APP/离线 APP 均**无需重打包**——①③随下次桌面/APP 整包及下次热包签发时生效；②在下次 EXE/APK 打包时生效（此前缺证书/错证书会 WARN 放行，此后直接 build_fail）；④随推送立即在 GitHub PR 检查生效；⑤为仓库门禁，立即生效。dingzhi 定制更新通道本轮冻结未动。

## 26. P1 B 批：心跳 KV 降写 + 工单激活通道对齐主审核（2026-09-24，纯 functions 服务端，五端零重打包）

* **写放大账（先量后改）**：心跳每台每 10 分钟一次，原稳态 **3 次 KV 写/心跳**：①license 整记录 get-merge-put（设备级 lastHeartbeat，在线统计数据源，必需）；②device_version 绑定**无条件** put（`setDeviceVersion` 还每次把 `boundAt` 刷成心跳时间，污染"绑定时间"语义）；③reportUsage **无条件** put 刷 lastReport（高水位没变也写）。另每 7 天匹配路径还多一次 updateLicense（同请求第二次写）。20 台活跃约 8.6k 写/天 → 改造后约 2.9k。
* **降写四刀（heartbeat.js + license-core.js）**：①`updateLicense` 新增可选 `__devicePatch`（machineId/lastHeartbeat/productClass/clientClass），内部重读最新 record 经 `getDevices()` 只 patch 本设备槽位——旧调用方（全仓 19 处）零影响，旧 machineId 单值格式借合成槽位完成等价的数组迁移；②纯心跳 **5 分钟近重复节流**（HB_WRITE_MIN_MS；正常 10 分钟周期恒过，在线窗口 15 分钟留 3 倍余量；metaChanged/needAudit 强制写；跳过=KV 不变，不可能造在线假象）；③device_version 改五字段语义 diff（version/productClass/clientClass/licenseCode/clinicName）全等不写；④reportUsage 高水位持平/月界时钟偏差直接 return 不落盘，**回拨（count<high）必写+count_rollback 审计**——风控只读审计日志出告警，lastReport 仅"已有告警"时作附属详情（admin-risk.js L302），安全信号零损失。
* **并发覆盖（非原子，措辞诚实）**：__devicePatch 把临界区从"整个请求多次 KV 往返"压缩为重读-修改-写一次 RTT，交错请求后写者必见先写者槽位；**KV 无 CAS，毫秒级同刻碰撞仍可能丢一次刷新，下个 10 分钟周期自愈**——注释严禁写"根治"。
* **★ 铁律：新增任何"激活/审批通过"通道必须复制主审核 admin-approve 三件套**：① **AR-01 停用诊所硬闸**（读 `KV_SYSTEM_CLINICS`='system:clinics' 同名诊所 `status==='disabled'` → 409 `code:'CLINIC_DISABLED'`；**检查异常 catch 必须 fail-closed 500**；闸位在 checkDeviceVersion 后、**任何写操作前**）；② licenseRecord 写 **firstActivatedAt 锚点**（buildLicenseData 有效期三级回退 firstActivatedAt→activatedAt→now；缺锚点=日后重激活续命，validate 只补缺失永不覆盖）；③ licenseRecord 写 **phone**（lookup 自愈/登录链；user 名字段不含手机号）。本轮工单通道 activate-from-ticket.js 曾三闸全缺=停用诊所可经工单复活（P0 级绕过缺口，与主审核同信任域 platform_admin 但两通道并列）。
* **对齐时不臆造源数据**：工单协议（ticket/submit.js）只有 edition/clinicName/contact*，**无 appMode/appModeCarrier/inviteCode**——devices[0] 端形态留空（审批请求 UA 是管理员浏览器，禁止嗅探！），靠客户端首次心跳 UA 嗅探/显式上报补空；邀请码结算不适用。判断"缺失"前必须先读提交端结构。
* **双审 P2 遗留 → 2026-09-24 已全部 resolved（见 §28 安全收尾批）**：①停用闸非数组 fail-open（三处显式闸 + provisionCloudAccount 落库点共四处统一 getClinicsOrThrow）；②工单审批 CAS 防双击双发；③手机号正则去掉 `\s`。
* **生效方式**：纯 Cloudflare Pages Functions，推送后 Pages 自动部署即生效；云端网页/云桌面/云 APP 的心跳与工单审批随部署生效；**离线桌面/离线 APP 对服务端改动透明，无需更新、无热包、五端零重打包**。部署后冒烟：两端点 OPTIONS 200（模块/import 错误会让 OPTIONS 也 500）、activate-from-ticket 无 token 403、心跳无效码 400；不做真实激活、不污染真实 license KV。commit `e3f1d6bd`。

## 27. C 项 #2：独立客服角色 service + 最小权限工作台（2026-09-24，纯 functions + 静态页，五端零重打包）

* **角色模型**：service 账号存独立 KV 表 `system:service_accounts`，仅 username 精确登录（建号正则禁手机号）；登录返回 clinicId:null、clientClass=web 天然跳过设备绑定；`isStaff=isPlatformAdmin||isService` 只允许出现在明确下放的端点上——**高权端点继续只用 isPlatformAdmin，禁止图省事把 isStaff 往外扩散**（本轮全 functions/ 仅 7 处 isStaff：6 个 license 端点 + account-diagnose）。工作台 public/admin/service.html 独立 sessionStorage 键 `service_console_token`（与 admin_console_token 物理隔离），tab 前端隐藏不算安全、服务端必须 403 兜底。
* **★ 铁律：新增角色必须同时交付「端点授权矩阵 + 独立 KV 表 + 独立审计 action」三件套**。service ALLOW 白名单=export-license / license list / ticket list / ticket reject / activate-from-ticket / admin-device-version（list/query/unbind）/ account-diagnose；service-account-list/save 仅 platform_admin；generate/batch/extend/update/status/admin-delete/admin-approve/admin-device-reset/admin-data-audit/order-confirm-paid 等全部维持 admin 403。每个 service 可达写操作必须留痕：`ticket_approve`/`ticket_reject`/`device_unbind`（平台级 writeAuditLog）+ export/unbind 命中码的 appendLicenseLog + `account_diagnose`/`service_account_create|update|reset_password`。审批通过是 service 可达最高价值写操作（发付费码+开云端账号），最初漏了平台级审计只在单码日志留痕——下放写权必查审计覆盖。
* **★ 下放既有高危端点时，必须重审"防滥用闸对新角色是否仍成立"，不能只改鉴权一行**。双审实锤 export-license 换机闸旧写法 `if(user&&originalUser&&user!==originalUser)` 两洞：①user 可省略→条件短路，任意 machineId 踢掉最旧合法设备；②user 对 service 在 list 接口明文可见、照抄即过（弱归属凭据）。修法按通道分级：**service 通道强制 user 必填+trim 后与授权登记用户完全一致，originalUser 缺失也拒**；admin/secret 通道维持旧行为（最小改动）。未使用/未满额码与同设备重激活不进闸（=离线激活业务本体）。
* **★ 频控按信任域分层**：IP 桶（可被代理池绕过、办公 NAT 共桶）之外，对 service 叠加**账号桶 30/h + 激活码桶 10/h**；checkRateLimit/checkCodeRateLimit 新增可选 prefix 参数（默认 `ratelimit:license`/`ratelimit:code` 不变，既有调用零影响），员工桶传独立前缀（`ratelimit:staffexport*`）——禁止复用 `ratelimit:code:` 与客户端 validate 的 5/h 桶共计数（客户重试会耗尽客服额度）。machineId 白名单统一 import schema-guard `isValidMachineId`（8-64），禁止在端点内另写内联正则（曾写成 8-128 口径漂移）；入口 trim 归一化后全文只可用归一化值。
* **★ 新角色必查"只认证不鉴权"的既有兜底分支**：change-password（本人）与 POST 无 action 的 doctor 自助兜底，对 clinicId:null 的 service 会落入 platform_admins 表 findIndex=-1 → **无 kv.put 却返回"成功"还 revokeAllUserTokens（自我 DoS+假审计）**。修法：两分支在任何 KV 操作前对 role==='service' 显式 403，客服凭证生命周期只走 service-account-save；统一救援端点 reset-password 补 service 写回分支，写回前 `Array.isArray` fail-closed、写回后 revokeAllUserTokens（只加在 service 分支，不得波及其余角色既有行为）。
* **service-account-save 四条写入铁律**：①KV 值存在但非数组 → 500 拒写（防新建分支用 [] 覆盖坏值抹掉全部账号）；②`disabled` 区分"未传"与"显式 false"（disabledProvided），漏传不得把停用账号静默启用；③停用/改密 revokeAllUserTokens，启用/重置/新建 clearLoginFailures；④role 服务端写死 ROLE_SERVICE、body 不接收 role，cur.role 异常拒写；建号全局查重走 findUserForLogin（已覆盖 D1/platform_admins/service_accounts/全部诊所，跨表同名在所有建号改名路径都会被拦）。
* **最小披露**：list 统一 sanitizeRecord（machineId 前 8 位）；ticket/list 服务端 6+6 脱敏；admin-device-version 对 service 三个分支全 maybeMask（≤12 位全打点），解绑凭客户提供的完整 machineId 执行；service-account-list/account-diagnose 不回哈希、不回同所用户清单。诊断失败计数与登录链 lockKey 同口径：原始 q 键 + 权威 username 键取 max（手机号诊断不漏读锁定态）。前端所有动态渲染经 esc()（statusTag 未知枚举值也必须过 esc）。
* **风险接受（已评估，不拦，勿重复立项）**：①account-diagnose 可查 platform_admin/其他 service 的存在性/姓名/停用/锁定态=内部员工目录可见性，排障所需且最小披露；②未使用/未满额码的离线导出无归属闸（离线激活业务定义，靠账号开通审批+三层频控+全程 license 日志（含 operator/authMethod=bearer-service/machineId 前 8 位）事后追溯；若未来出现滥用再升级"强制关联工单+服务端校验工单 machineId 一致/双人复核"，不提前做（离线客户本就无法在线提工单）。**运维动作（代码外）**：LICENSE_EXPORT_SECRET 旧值曾随 generate-license.ps1 入库 git，须在 Cloudflare 后台轮换；service 上线后可直接将该变量置空关闭密钥通道（代码 `if(exportSecret)` 已支持）。
* **双审方法论（授权链变更必走）**：两个互不通气子代理（功能正确性 + 安全攻击面）各自读码取证 → 主审对 High/分歧项亲自读码定修复范围（业务风险接受须显式记录）→ 修复后派第三名独立复审对照发现逐项验闭环+回归扫描，全绿才提交。本轮结果：初查 1 High（换机闸短路）+1 Medium（频控维度）+8 Low，修复 9 项+追加 2 项加固（频控独立前缀、日志截断），复审 9/9 闭环。
* **生效方式**：纯 Cloudflare Pages Functions + public/admin/service.html 新静态页（admin 页不在 sync-all/check-interface 基线范围），push 后 Pages 自动部署即生效；**云端网页即生效；云桌面/云端 APP/离线桌面/离线 APP 零改动、无热包、零重打包**。首个客服账号由 platform_admin 登录 `/admin/service.html`「客服账号」tab 创建（无 CLI/bootstrap；KV 键不存在读为 [] 属正常）。部署后冒烟：service.html 可打开；无 token 调 staff 端点 403；service token 调 generate 等未放开端点 403；不做真实发码、不污染真实 KV。

## 28. 安全收尾批：停用闸 fail-closed 单一权威入口 + 工单 CAS 状态机（2026-09-24，纯 functions + admin 静态页，五端零重打包）

* **★ 铁律：读"整张权威清单"类 KV（诊所表 system:clinics 等）禁止再抄 `(await kv.get(k,'json')) || []` + `Array.isArray(x)&&x.find()` 模式**。键不存在读 null=空表是合法语义；但"值存在且是合法 JSON 却非数组"（对象/字符串/数字，误写/备份恢复污染）时 find 静默不执行=安全闸 fail-open。唯一权威入口 `getClinicsOrThrow(kv)`（functions/api/_lib/auth.js）：**null/undefined→[]（空表）；非数组→throw ClinicsDataError；kv.get/JSON 异常原样上抛（不 catch 不回退）**；配套 `findClinicByName(clinics,name)`（空名 undefined、元素 null 有 `c&&` 防护）。本轮四处接入（缺一处即通道分叉）：admin-approve/activate-from-ticket/admin-submit 三个 AR-01 显式闸 + **provisionCloudAccount（admin-account.js）落库点**（最危险：旧写法非数组落入"新建诊所"分支 push 后整表 put=覆盖全平台诊所清单）。调用方 catch 口径：审批点 500/工单点回滚后 500、**匿名 admin-submit 503 且错误信息禁止 fail-open 放行**；其 6 个 provision 调用方全部已有 try/catch（validate 仅语音升级分支调用，主激活链不受影响）。
* **★ 铁律：匿名/低信任入口的安全检查异常必须 fail-closed，禁止 catch 后"放行业务"**。admin-submit 旧 catch 注释"放行申请"=停用诊所可匿名复活，现 503 `CLINIC_CHECK_ERROR` 固定文案（客户联系微信 hktzy1688 兜底）。KV 瞬态错误罕见且外部无法触发对固定键的污染，安全提升不构成可被匿名利用的 DoS；admin-status 轮询通道不经过该闸，老客户仍可凭轮询拿 license。
* **★ 铁律：KV 上的"人工审批发码"类写操作必须用 CAS 四件套逼近互斥**（KV 无原子 CAS，不写"根治/彻底防并发"，措辞诚实）：①**processing 占用中间态**（pending→processing→approved/rejected，processingBy 服务端取自 token 用户名、processingAt 服务端时钟、processingToken=16 字节 crypto 随机 hex——body 永远不并入工单 JSON）；②**写后复核**（claim put 后立即回读，status/令牌不符即弃权）；③**不可逆动作前终审**（生成码/开通账号前再读比对令牌）；④**终态写前终读复核**（approved 盲 put 前最后一读，不符则 409 TICKET_PROCESSED_ELSEWHERE 不覆盖终态+`ticket_approve_terminal_conflict` 审计含双方 code）。配套：参数校验全部前移占用之前（非法 400 不留残留）；占用后每条失败路径必须经**令牌双校验（status==='processing' && processingToken===自己）的读改写回滚** pending；**回滚闭包必须在 claim put 之前定义并赋值句柄**（连 put reject/复核 GET 瞬断都能被外层 catch 回滚）；外层 catch 统一回滚；崩溃残留 **10 分钟 TTL 可接管**。
* **TTL 时间戳口径**：processingAt 缺失/非法(NaN)/未来时间(负)两侧（activate/reject）一律判"可接管"（此类污染仅 KV 权限级可达；信任 status 而非可脏时间戳，杜绝永久卡死）；正常 0-10 分钟双方一致拦截。
* **防孤儿码**：`let issuedCode` 在 saveLicense 成功后才赋值；码已发之后的崩溃回滚 reason 与 `ticket_claim_rollback` 审计必须带 code，人工凭 audit + license 日志回收（provision 按诊所名幂等，重发不会重复开户）。
* **审计**：接管（`ticket_claim_takeover`，activate/reject 双写，含 prevProcessingBy 不含令牌）、回滚（`ticket_claim_rollback`）、终态冲突（`ticket_approve_terminal_conflict` / reject 写后复读仅在收敛为 approved 时告警 `ticket_reject_terminal_conflict`，陈旧 pending 读不告警防噪声）。令牌字段（processingToken/By/At）禁止进入任何输出白名单（ticket/list.js 字段白名单实证不含；全仓仅 activate/reject 两文件引用；无匿名工单详情端点）。
* **错误响应最小披露**：ClinicsDataError.message 含内部 KV 键名，只进 console.warn，响应体统一固定文案 + code（admin-approve/activate 500 CLINIC_CHECK_ERROR、admin-submit 503）。
* **输入收紧**：ticket/submit 手机号 `/^[0-9+\- ]{5,20}$/`（**去掉 \s，只留普通空格**，\n\r\t\v\f 全挡）+ `replace(/\D/g,'').length>=5`（防"+++++"式零数字）；无 u 标志只匹配 ASCII（全角/nbsp 天然拒）。服务端单点收紧即可，不同步客户端（旧客户端脏数据被服务端 400 拒，无需发版）。
* **前端配套（admin 页不在 check-interface 基线）**：service.html 增加 processing 筛选项/⏳标签/处理中行无操作按钮，ticketAction `if(btn.disabled)return;btn.disabled=true;` + **finally 恢复**（confirm/prompt 在 try 内取消也必走 finally）；public+site-admin 两个旧 ticket-approval.html 加 processing 标签（按钮门控本就只认 submitted/pending，服务端 409 兜底）。
* **残余风险（已评估接受，勿重复立项）**：跨 colo 最终一致陈旧读理论上可使两请求都过终审产生两个有效码（需 staff 权限+两 colo+多处陈旧读同时成立；同浏览器双击落同 colo 必败）；失败方向安全（误判即弃权不发码）、双 ticket_approve 审计+license 日志可事后发现、detach 使设备最终只绑一码；未来真出现再上 Durable Object 按工单 key 串行化，当前不提前做。
* **方法论补充**：本轮双审两子代理独立交叉命中同一高价值问题（回滚句柄时序），初查 5 Low（功能）+4 Low/1 中（安全，中危即上述接受的残余窗口），修复 7 项+加固 2 项（句柄赋值再提前至 put 前、reject 写后冲突审计），第三名复审全路径表逐项验闭环后才提交。
* **生效方式**：纯 Cloudflare Pages Functions + admin 静态页，push 后 Pages 自动部署即生效；**云端网页即生效；云桌面/云端 APP/离线桌面/离线 APP 零改动、无热包、零重打包**。部署后冒烟：ticket/submit 无 body→400、手机号含换行→400、正常 11 位手机号进入后续校验；activate-from-ticket 无 token→403、OPTIONS→200；不做真实审批发码。

## 29. P1-2 客户全景聚合端点 customer-aggregate（2026-09-25，纯 functions + admin 静态页，五端零重打包）

* **只读聚合端点四件套**（users.js `GET /users?action=customer-aggregate`，8 分区：账号/墓碑/付费码/设备绑定/设备封锁/进行中订单/工单/官网订单申请/免费领取/诊所视图）：①分支首语句 isStaff 403，先于一切数据读取；②频控独立账号桶（`staff-aggregate:{username}` 60/h，前缀 `ratelimit:staffaggregate`，KV 异常 fail-closed 500）；③审计走 waitUntil，extra **只记 queryType 与各分区命中计数/布尔，不记原始 machineId**，target=q（客服本人输入）可记；④除频控/审计外**零业务 KV 写**（findUserForLogin 的 D1 幂等回填是同值自愈不算）。
* **★ 铁律：查询类型判定必须处理"字符集重叠"**。machineId 正则 `[A-Za-z0-9_-]{8,64}` 与 ≥8 位 ASCII 登录用户名（zhangsan 等）完全重叠，简单 phone→machine→name 三级判定会把这类用户名误判 machine，导致付费码/工单/订单按机器码匹配而**静默漏数**（账号分区仍命中，极具误导）。正确顺序：手机号（归一后）优先 → machine/name 暂定 → findUserForLogin 精确命中 `canonicalName===q` 时 machine 改判 name，且改判点必须在任何 queryType 消费分区之前。
* **★ 铁律：跨分区手机号比对必须双边数字归一**。工单写入端允许 `+86`/空格/连字符（`/^[0-9+\- ]{5,20}$/`），严格等值会漏掉客户的 `138-0013-8000` 工单。归一函数：`replace(/\D/g,'')` 后先匹配 `^1[3-9]\d{9}$` 再匹配 `^86(1[3-9]\d{9})$` 去 86；比对两侧都归一（licenses/tickets/adminRequests），free_pass 等精确键用归一后 11 位号码；账号查找/墓碑/失败计数键同样用归一值（lookupQ=phoneQ||q），否则出现"订单命中、账号不存在"的跨分区撕裂。
* **★ 铁律：新端点禁止直接复用 sanitizeRecord 的脱敏口径当承诺**。sanitizeRecord 输出 machineId **前 8 位**+`...`，对恰好 8 位的短机器码=整值泄露；聚合端点必须用与 ticket/list.js 同口径的 maskMid（String 化防数字毒数据，≤12 位全打点，否则前 6+••••+后 6）重写主码与 devices[] 每台设备（两次 getDevices(r) 纯函数等长同序，下标对齐）。device_block 不回 lastIp/codeHash/user；active_order 等 KV 直读记录**必须重建白名单对象**（5 字段），禁止整条透传防历史脏键带出额外字段。
* **★ 纯展示分区容错 ≠ 安全闸 fail-closed**：诊所视图用本地 Array.isArray 守卫 + try/catch（清单损坏只空该分区，不得拖垮聚合响应）——这与 §28 getClinicsOrThrow 的安全闸语义相反，两类场景不得抄混。账号(findUserForLogin)、license 索引等分区同样 catch 降级 null/[]。
* **★ 扫描类分区必须同时返回"截断标志 + 索引损坏标志"**：CAP=150，`xxxTruncated: indexValid && raw.length>CAP`（短路防 null.length）、`xxxIndexValid: Array.isArray(raw)`。前端 warn-box 必须区分"更早记录未纳入"与"索引损坏本次未扫描"——"无数据"和"没扫成"对客服是两种结论。空分区在页面底部汇总明示（无墓碑·无工单…），phone 查询注明未付款官网单不在 admin_req_index 扫描范围。
* **★ 全扫 KV 读用分批并行 batchGetJson（批 25）**：子请求总数不变，墙钟从 N×RTT 降到 ceil(N/25)×RTT（300+ 次串行读 3-15 秒→亚秒级）；out 下标按 `i+j` 对齐、每键 `.catch(()=>null)` 毒键落 null 不拖批。子请求总量仍随码数/诊所数线性增长，~700 条撞 Workers Paid 1000 上限——**根治靠 P2-5 mid→code 派生索引改 O(1)，届时本端点与 listLicenses 全扫一并切换**。
* **active_order 展示侧 48h 过滤**：getActiveOrder 直读无年龄判断、写入无 TTL，超 48h 僵尸待付单（admin-data-audit 的 stale_active_order 巡检对象）不得在聚合中当"进行中订单"展示；用 ACTIVE_ORDER_MAX_AGE_MS 比对 createdAt，非法日期(NaN)比较落 null。
* **admin 静态页规范**：新 Tab 三处配平（按钮 data-tab / pane id / switchTab 数组）；只复用现有 class（card/dl/tag/warn-box/msg/table-box）不加 CSS（admin 页不在 check-interface 基线但界面保护铁律适用）；全部动态值过 esc()（含 statusTag 未知枚举兜底）；查询防连点=按钮 disabled+finally **再加 aggLoading 布尔**（disabled 拦不住 Enter 键）；输入框 maxlength 与服务端 slice(0,64) 对齐。
* **双审记录**：初查功能 2 High（ASCII 用户名误判 machine、格式化手机号工单漏配）+4 Medium（串行读/分区无 catch/状态枚举缺 3 个/僵尸订单）+8 Low，安全侧 4 Low（8 位脱敏/activeOrder 透传/子请求放大/fmtDate 回退）；修复 11 项后第三名复审 8/8 闭环，复审又补 3 个一行级 Low（lookupQ 归一、licensesIndexValid、r.user||r.username 历史字段兜底）全部当场收口后才提交。
* **生效方式**：纯 Cloudflare Pages Functions + public/admin/service.html 静态页，push 后 Pages 自动部署即生效；**云端网页（客服工作台）即生效；云桌面/云端 APP/离线桌面/离线 APP 零改动、无热包、零重打包**。冒烟：无 token→403、q<5 位→400、OPTIONS→200、q 超长不报错；不做真实客户数据拉取。

## 30. P1-6 dingzhi 陈旧更新通道收口（2026-09-25，纯发布工具链+静态 JSON，五端零重打包）

* **四层残留模型（看到 dingzhi 先分层，禁止一刀切）**：① **HTTP 更新目录** `public/updates/dingzhi/`（+site-official 镜像）= 2026-08-23（9762ac9a）改名前桌面 EXE 硬编码的旧检查 URL，属 legacy 跳板；② **hash-manifest.json 的 `dingzhi` 键** = 发布工具 APP_CONFIG 内部键名镜像，离线 APP MainActivity（L1039-1040）保留 `local` 键缺失时 fallback 读它的兜底链——**只读兼容镜像，禁拆禁新增消费方**，随 APP 整包淘汰 fallback 后才可删；③ **工具渠道参数旧名**（rollback/update-manifest 的 dingzhi 入参）= 兼容别名；④ **内部代号**（artifact-locate APPS 键、release-menu `--target=dingzhi`、calculate-hash 键、Electron session partition `persist:tcm-prescription-dingzhi`、Android 包名）= 活跃构建身份，**全部保留不动**。
* **跳板策略（不是删除是引渡）**：legacy 目录直接删 → 极老客户端检查更新 404 静默、永久收不到提示；保留停更内容（曾停 1.0.172 三周）→ 老客户只能升到中间旧版。正确做法：**每次 publish-release 把 `updates/local/latest.json` 字节复制为 `updates/dingzhi/latest.json`**，再随既有递归镜像同步 site-official（失败=发布失败 exit 1；local 投影缺失必须 WARN 不得静默）。老客户端一跳直上最新版，新版 main.js 读 local 永久归队。**撤销条件**：确认无 2026-08-23 前构建桌面端在网（或满一个大版本周期）后删双站 dingzhi 目录与复制逻辑。
* **★ 铁律：任何"回滚/重投影 latest.json"操作必须维持与发布链相同的两处不变量**——① local 渠道同步 dingzhi 跳板；② public/updates→site-official/updates 递归镜像。git add 用精确文件白名单（local 4 条/cloud 2 条），镜像后另跑 `git status --porcelain` 对 addPaths 之外的漂移大声 WARN。历史坑：rollback.js 旧配置写 local 文件却 git add updates/dingzhi 路径，--push 必失败；回滚备份一律写 `os.tmpdir()`，禁止仓库内 *.bak（code-quality CI 判红+会被镜像带走）。
* **★ Windows 工具链铁律：Node 调外部命令禁止 execSync 字符串模板，一律 execFileSync + 参数数组**。本轮双审后复审实锤：拼给 gh --jq 的正则并集 `huikang-(?:local|dingzhi).*\.exe$` 中 `|` 被 cmd.exe 当管道符（local 渠道确定性失效），且 jq 字符串里 `\.` 是非法转义（cloud 渠道历史同款 bug，两渠道都列不出版本，长期无人用过回滚工具）。数组传参同时消灭这两类问题与 owner/repo 注入面；jq 侧正则转义 JS 写法 `source.replace(/\\/g,'\\\\')`，JS 正则的 `i` 标志要显式写 `test(pattern;"i")`（jq 不继承 JS flags）。
* **回滚资产 pattern 用两代并集** `/huikang-(?:local|dingzhi).*\.exe$/i`：huikang-dingzhi-* 资产 48 个（≤1.0.158，实测 1.0.0–1.0.158）真实存在于 Releases，改名不删历史，回滚可见性必须保留；新资产 1.0.159 起全 huikang-local-*。
* **残余风险（已评估接受，勿重复立项）**：≤1.0.24 旧 update-notifier 消费跳板 JSON 的 url 无 host 白名单、JSON 无 sha256 即不校验完整性——但可写该 JSON=已具 main 推送/Pages 部署权=可同时改 local/hash-manifest，分发链已全面失守，跳板不构成独立入口；续期只提高沉寂老客户端的暴露频次，增量风险≈0。不为此加 sha256（老 notifier 单哈希会误伤存量便携版：setup/portable 两包无法共用一个哈希，便携客户端校验失败反升级受阻）。老客户端一跳跨约 230 版本（1.0.172→1.0.255 同方向逐版升级的最终状态一致，取消 admin/admin、强制注册等变化自 1.0.172 时代已逐步生效，非本次新增跳变）。
* **双审记录**：功能侧无 High（1 Medium=回滚不维护跳板/双站，4 Low）；安全侧无 High（Medium-1=上述旧 notifier 残余风险接受，Medium-2 同功能侧，Low 含注释失实/.bak/静默跳过）；修复 7 项后第三名复审复跑又实锤 1 High（cmd.exe 吃正则 `|`）+1 既有 Medium（jq `\.` 转义），改 execFileSync 数组传参后两渠道 gh API 实测：local 列出 1.0.0–1.0.255 两族资产、cloud 列出 1.2.0–1.2.254 零串渠道，复验闭环才提交。
* **生效方式**：纯发布工具脚本 + 两个静态 latest.json（public 与 site-official 各一），push 后 Pages 自动部署；**五端零重打包、无热包**——现役云/离桌面只读 cloud/local 通道行为不变；仍在网的 2026-08-23 前极老离线桌面下次启动检查更新即收到 1.0.255 跳板（客户端侧无任何变化）；云端/离线 APP、云桌面完全不读该目录。

## 31. P1-7 客服材料对齐现状（2026-09-25，纯文档+客服脚本文案，五端零重打包）

* **改动 2 文件**：① `docs/客服离线激活操作手册.md` v1.0（2026-07-25，curl/Postman 教粘密钥）整体重写为 **v2.0**——首选入口改 service 客服工作台（`/admin/service.html` 7 Tab：工单/离线激活/授权查询/设备解绑/账号诊断/客户全景/客服账号[仅管理员]，能力边界对齐 §27 service ALLOW 白名单），脚本降为备用通道，curl/Postman 明示废弃；② `generate-license.ps1` 三处过时文案「客户32位机器ID」→「完整机器码（8-64 位英文/数字/_/-）」（占位符/操作提示/错误对照各一），仅文案不动逻辑，PS Parser 校验通过。
* **手册口径全部回源代码取证（不凭记忆）**：机器码以 export-license.js+schema-guard 的 `isValidMachineId` = **8-64 位英文/数字/下划线/连字符**（旧手册正文 32 位、错误表 8-128 两版都错）；密钥 64hex（`^[0-9a-fA-F]{64}$` 本机文件）；频控三层=**所有通道 IP 20/h（`ip:export`）+ service 账号 30/h（`ratelimit:staffexport`）+ service 单码 10/h（`ratelimit:staffexport:code`）**，secret/admin 通道只有 IP 单桶；换机闸 service 通道强制 user 必填且与登记用户完全一致（三句拒绝文案原文照抄）；`licenseInfo.type` 补 `free`；错误表逐条抄线上 error 文案（含 needClinicName 400、诊所不一致 403、禁用/过期/不存在）。
* **客服对客口径（手册第一章，与 download.html 一致）**：注册账号制（手机号即账号、无默认 admin/admin）；7 天试用 / 永久免费版（激活窗口【免费使用】、本机永久、开方不限量，备份恢复/Excel/拍照录像/语音为标准版、打印带水印，输码升级数据不丢）；**语音录入任一付费版均含、无单卖**；客服微信全产品唯一 **hktzy1688**（旧号 huikang-tcm 仅以"已停用"出现在手册；全仓除 archive 历史日志外无残留）。
...
## 33. P2-2 核心业务冒烟 biz-smoke + loadData 毒数据实锤修复（2026-09-25，测试资产+门禁+一处六端修复）

* **测试资产**：新增 **`tools/biz-smoke.cjs`（86 断言，零依赖 node:vm + ESM 动态 import）**，pre-push 升级为**十三道门**（⑬ 业务冒烟，秒级）。四组：
  * **A 库存扣减/冲正（15 条）**：vm 裸沙箱加载 **shared/stock-core.js 真实 IIFE**（mock localStorage/DOM 桩），经其自带 `_planDeduction/_commitDeduction/_revertRecord` 测试钩子（作者本就预留）。锁定契约：剂量×剂数、同名药味合并、同内容重存零差值（`__stockApplied` 幂等 marker）、减量补退/删味全退、无 marker 不幻影退回、药已删改名跳过不抛、库存可扣负（开方不阻断只预警）、round2 浮点（0.1×3=0.3）、流水类型 rx/rx-adj/rx-revert（**rx-adj 的 qty 是带符号 delta，退回记 -21；流水 unshift 最新在前**）。
  * **B 续费数学（11 条）**：`buildLicenseData` 锁定 2026-08-26 定版规则——锚点 firstActivatedAt→activatedAt→now，重装/换机不续命；rewardDays 合法叠加；`record.expiresAt` 仅在晚于锚定到期时保底取晚者；坏日期不采用；days 非法默认 365。测法传 `{secret:'x'}` 走 HMAC、不传 context 跳 ECDSA 段。
  * **C 处方 D1 毒数据（7 条）**：`d1RowToPrescription/safeJsonParse`——坏 JSON 列（items/media_files/extra）回退默认值；**纯数字串 id 还原 Number 是 2026-09-12 "删除/加载按钮失效"事故的回归锁**（D1 TEXT id vs 客户端时间戳数字严格相等）。
  * **D loadData 毒数据（六端×6+存在性）**：用 smoke-runtime 同款字符串感知扫描器，从 sync-shared-blocks 的 **HTML_FILES 六份真实产物**提取 `safeParseJSON/removeDuplicateMedicines` 函数体 vm 执行——既测行为又守副本漂移：哪份副本缺函数/漂移/抛错即 FAIL。
* **★★ D 组建测当天实锤真实事故面（六端同款）**：`removeDuplicateMedicines` 旧实现 `m.name+m.code` 不防 null，而 public/index.html 药品加载调用点（约 L6050 `medicines = removeDuplicateMedicines(parsedMedicines)`）**无 try/catch**（另一处 L6061 有）——localStorage/迁移边缘产生的 null/非对象药条目会让整个 loadData 抛错中断（历史处方+药品全空，1.2.101 静默崩溃同类）。修复仅一行守卫 `if (!m || typeof m !== 'object') return false;`，**六端分发走全正规链**：public/index.html 权威源 → sync-html.ps1（云桌面/云 APP assets）；db-offline/index-app.html 离线 APP 权威源（sync-all 31 变换生成离线 APP assets）；db-offline/desktop/index.html 手工对端（diff-cross-version desk 对强制字节一致）；site-admin 标记块走 sync-siteadmin.cjs。修复后 sync-all/html-sync/check-interface(6 OK)/cross-ver/cv-hash/smoke 157 全绿。
* **续费数学收口**：users.js clinic=update 续费段（原 3 行内联）抽为 **`export function computeRenewedExpiresAt(curIso, days, nowMs=Date.now())`**——未过期从到期日续（剩余天数不损）、已过期/无值/坏日期从今天续；原位替换行为等价（NaN 处理一致）。Pages Functions 加 named export 不影响 onRequest* 路由，Node 动态 import 可直接测。
* **★ 测试方法论（复用）**：① 生产 IIFE 预留 `_` 前缀测试钩子的模式在 stock-core 已验证好用，后续 shared 模块照做；② 埋在 HTML 的内联纯函数不必为测试抽模块——**提取函数体 vm 执行**同时拿到"行为锁+副本漂移锁"，与 smoke-runtime 同范式；③ 业务断言先写现状契约（哪怕符号/顺序反直觉），实锤缺口再修生产——本轮 D 组就是测试先红、修复后绿。
* **生效方式**：biz-smoke.cjs + 十三道门随 push 生效（开发流程资产）；`removeDuplicateMedicines` 修复在各端 index.html **内联**——云端网页 push 即生效；**云/离桌面与双 APP 的 index.html 随整包，热更不可达（无 HTML 热包）**，下次各端发版自然带上（触发条件=药品数组已含 null 脏条目，正常使用不产生，风险低；不为此单独发版）；users.js 续费纯函数行为等价，CF 部署即生效。

## 34. P2-1 统计块抽 shared/analytics-core.js（2026-09-25，消双份内联，五端中网页/桌面/离线APP 热更可达）

* **产出**：**`shared/analytics-core.js`（634 行 IIFE，11 函数）**——getFilteredPrescriptions/analyzePatients/analyzeDiseases/analyzeMedicines/analyzeVisitTrend/analyzeMonthlyStats（async）/refreshAnalytics（async）/exportStats（async）/loadPatientHistory/formatDate/createChart（canvas 手绘柱/饼/横条，DPR+响应式+重试）。云端 public 与离线 desktop 两份内联块（各 ~580 行）字节级同构（`git diff --no-index` 仅 2 处差异），抽取后两端内联删除、插入 `<script src="analytics-core.js?cv=04542dfb">`（离线/鸿蒙无 cv）。
* **★ 平台分叉自适应（勿做双副本）**：唯一真分叉是离线 exportStats 多一道付费墙。模块统一 async + `if (typeof global.requireFeature === 'function') { if (!await global.requireFeature('data-export')) return; }`——网页端无该函数自动放行，离线端保留付费墙。以后遇"仅某平台多一道闸门"类分叉，先想 typeof 自适应而非复制文件。
* **跨作用域裸标识符（同 stock-core 范式，禁用 new Function/eval——云端 CSP 拦 unsafe-eval）**：prescriptionHistory、medicines、getAllUserPrescriptions、filterPrescriptionsByPermission、escapeHtml、escapeJs、formatPrice、closeModal、searchPatientsQuick、showSideHistory。模块在主脚本之前加载，函数仅在调用时解析标识符，故安全。自安装：refreshAnalytics/exportStats/loadPatientHistory 挂 window + 暴露 window.AnalyticsCore。
* **基础设施登记（9 处，新 shared JS 的完整清单照此办理）**：sync-all.ps1 `$BusinessJsFiles`（9 文件）/ copy-consistency.cjs 新组 **8 副本**（public、public/electron、cloud_desktop、cloud_desktop/electron、cloud_app assets、offline desktop、offline app assets、harmony rawfile）/ check-cv-hashes TARGETS（现 **15 项**）/ generate-desktop-hotupdate 云+local 两通道清单 / generate-app-hotupdate FILES / public/_headers immutable / 两桌面 package.json build.files。cv 经 `check-cv-hashes --update` 写真哈希，禁手填。
* **site-admin 处置（Tier A 降级路径）**：8 个函数权威源移走后，从 sync-siteadmin.cjs SYNCED_FNS **移除（69→61）**，site-admin/index.html 对应 SYNCED-FN 标记块**剥外壳保留函数体（成为自持函数，零行为变更）**，再 `diff-cross-version --update-baseline --pair siteadmin`（10 个统计函数重分类为仅 B，site-admin 不在界面锁内，剥标记不影响 check-interface）。**不做"site-admin 加载 analytics-core"方案**：其 refreshAnalytics/analyzeMonthlyStats/exportStats 本就是独立分叉（Tier C 永久否决），整站是手工双轨遗留。
* **鸿蒙手工盲区**：rawfile/index.html 同样删块+插引用（stock-core 在该页也无 cv，照其惯例），JS 手工复制——copy-consistency 只验证存在与哈希，不会帮你生成。
* **验证（全绿）**：sync-all VerifyOnly / copy-consistency 8 副本 / cv 15 项 / check-interface **6 OK** / diff-cross-version 三层基线 / biz-smoke **86/86** / smoke-runtime **157/157** / node --check；**浏览器真实冒烟**（本地静态服务+localStorage 降级源注入 8 条跨日期处方）：弹窗打开、本月 4 患者/初2复2/50%、三张 canvas 实际像素 16.8K/26.8K/8.9K、today/week/year 过滤数学全对、月度表剂数（9月21/8月3=处方数×3）、导出按钮无错、**控制台 0 error**。
* **★ 方法论**：① 浏览器 evaluate 注入数据可直接写全局词法绑定（`prescriptionHistory = A`，顶层 let 跨经典脚本共享），但脚本里**禁用 IIFE**（引擎自动包裹，返回 undefined）；含 for/块语句时完成值也会丢，先挂 window.__x 再二次读取；② 抽模块后凡有"标记块/基线/cv"三类登记的工具链都要全查一遍，漏掉任何一处 pre-push 才报错是最低档，理想是改完即自检。
* **生效方式**：**云端网页 push 即生效**（_headers immutable 走 cv 破缓存）；**云桌面/离线桌面 desktop 双通道热包可达**（联网重启 2 次）；**离线 APP app-local 热包重启生效**；**云端 APP 随包/实载线上即时**；鸿蒙随包。旧 index.html 内联块已删，热更后磁盘无残留双份（新模块覆盖式新增）。热包必须在 commit 后重签三渠道并线上核验 version.json 哈希。
* **★ 同日追加修复（2026-09-25）loadPatientHistory 补 target 参数**：原 `searchPatientsQuick({key:'Enter'})` 缺 target → 防抖 300ms 后搜空串，把侧栏覆盖成"请输入患者姓名"（"调取处方"按钮的真实用户可见故障：点完看不到患者病历）。修为 `const nameInput=document.getElementById('patientName'); nameInput.value=patientName; searchPatientsQuick({target:nameInput,key:'Enter'})`——用真实 input 元素作 target，与真键盘事件同形。**两处源头一起改**：shared/analytics-core.js + site-admin 自持副本（S1 冻结分叉也要顺手修，否则管理台留同样故障）；新 cv=**32994144**（sync-html 传播云副本）；云APP assets/鸿蒙两个手工盲区照旧手动复制；热包重签为 **2026.09.25-2**（三渠道）。浏览器实测：调用后侧栏 3 条张三病历日期倒序、占位语消失、0 error。教训：调"伪事件"函数（event 参数）时必须构造被读取的全部字段，写代码时直接 grep 函数体确认，别只凭调用习惯。

## 35. P2-3 打印域抽 shared/desktop-print.cjs（2026-09-25，主进程域，需双桌面重打 exe）

* **产出**：**`shared/desktop-print.cjs`（工厂 createDesktopPrintIpc({ipcMain,BrowserWindow})，138 行）**——`print-prescription` 单个 IPC handler：隐藏 BrowserWindow（show:false/600×850，横版 820×600，contextIsolation:true+nodeIntegration:false 无 preload）→ 正则剥离 @page → Buffer base64 编码 data:text/html URL 加载 → did-finish-load 后等 500ms+字体 ready+200ms → webContents.print（silent:false 手动选机/A5/无边距/横版参数）→ safeResolve 关窗；did-fail-load 返回 false，2 分钟超时硬兜底。两份 main.js 各删 115 行内联块、同位替换为一行 require 接线（app ready 前注册，时序等价）。
* **★ 改前哈希审计（高风险链强制流程）**：改前先把两块抽出行级哈希存档——双端 115 行 SHA256 均为 **`cd537a37…f45b6`，diff=0**；抽取后复审独立实测：工厂体内 115 行同哈希、与 HEAD 两块**逐字节相同**（含 blob 与工作副本 LF 一致性），完整文件 3 份（shared+2 electron 副本）SHA256 一致（6bc9f575…c116）。以后主进程域抽取照此办理：**先存改前块哈希 → 工厂体生成（禁手抄）→ 复审比对工厂体哈希==改前哈希**。
* **分发/基础设施**：sync-all **Group 21**（$DesktopPrintTargets=2 electron 目录；注释勿再写成 Group20——20 是 video-recorder）+ copy-consistency 新组 desktop-print（2 副本硬哈希门，总 **96 副本/21 组/0 失败**）；package.json `electron/**/*` 通配已覆盖无需改；check-ipc 合并扫描 electron/*.cjs 后 handler 不丢（云 70 注册/离 82 注册）。
* **测试资产**：新增 **`tools/desktop-print-smoke.cjs`（零依赖 stub electron，24 断言）**——注册面、竖/横窗尺寸与 webPreferences、@page 剥离+内容完整、打印五项选项、成功/打印回调失败（现状契约仍 resolve true）/did-fail-load/ctor throw 五回路、关窗时序。全门禁：node --check/IPC 一致/biz-smoke 86/smoke 157/界面 6 OK 全绿；**真实 Electron 直启回归双端 PASS**（BNZC_E2E=1+marker+BNZC_E2E_DATA 隔离 userData，dev electron 直启源码 main.js 免重打包：云端到"软件激活"窗、离线到"登录"窗，进程 13s 存活、窗口标题正常，taskkill /T 树杀+清 marker/tmp）。
* **独立安全复审（新上下文，零阻断/零重要）关键结论**：base64 字母表保证渲染方 html 无法注入 dataURL 头/逃逸协议；data: 文档 opaque origin + 无 preload + nodeIntegration:false → 打印窗无 ipcRenderer/Node 能力，恶意 html 至多普通网页能力，抽取未引入新能力；2 分钟超时 setTimeout 在主进程页面无法清除，IPC 不可被挂起；工厂注入面最小，模块内零 require/process/global/__dirname 引用。
* **★ 加固 Backlog（复审建议 1–4，均为 HEAD 既有、非本次引入，等体收口禁止夹带；下次打印域立项再改 shared/desktop-print.cjs）**：① `safeResolve` 的 close() 可被页面 beforeunload 阻止（IPC 返回不受影响，仅窗口残留）→ 加 `webContents.on('will-prevent-unload', e=>e.preventDefault())`；② 打印窗无 setWindowOpenHandler/will-navigate 拦截，恶意 html 可弹窗/导航（威胁模型=渲染方被攻破，风险低）；③ 未显式 sandbox:true（可与 nodeIntegration:false 双保险）；④ 返回值不反映打印成败（取消也 resolve true），渲染方不能据布尔判断。
* **生效方式**：**主进程域热更不可达**——云桌面/离线桌面需各自**重打 exe**（可与 license IPC 抽取等后续改动攒一批一起发）；云端网页/双 APP/鸿蒙不受影响。改前工作区仅未跟踪 promo/，不混入提交。

## 36. license IPC 胶合层抽 shared/desktop-license-ipc.cjs（2026-09-25，主进程域，需双桌面重打 exe）

* **产出**：**`shared/desktop-license-ipc.cjs`（工厂 createDesktopLicenseIpc(options)）**——两端 main.js 全部 license:* IPC 收口，**38 通道**：通用 22（21 字节同构 + submit-activate 一行推广注释差异取云端版）/ 端分叉 3（get-status、can-prescribe、submit-order-direct，两版行为都保留按 productClass 选）/ 仅云端 1（load-pending-order-no）/ 仅离线 12（verify-gate、gate-failed、install-admin-license、query-order-status、install-from-server、set/get-flow-state、start-trial、claim-free、register-local-user、load-registration-info、get-activation-users）。
* **★ 非同构块抽取范式（打印域 115 行同体范式的升级）**：先逐 handler 切片（块结束=行首 `});`）按通道建图→打印"同构/差异/仅xx"清单与改前大块哈希→工厂按"通用 + if(productClass) 分叉 + 条件注册组"组装，**handler 字节禁手抄、禁重缩进**（包住的块保持原缩进，才能持续做哈希取证）。改前三块哈希：cloud L828-1187 **d22ffd05**…、local L1043-1502 **27e7ae79**…、local 注册块 L2011-2168 **6f18ca79**…。
* **★ 自由标识符闭包规则**：handler 里对窗口的引用统一做机械变换 `mainWindow → getMainWindow()`（惰性访问器与外层可变 `let mainWindow` 等价；对齐 desktop-windows/desktop-dialog 的 B2-2 范式，**禁止把窗口提前 const 缓存**）；模块解构 16 个依赖（通用 10 + 离线 6：isTrialDenied/fse/safeStorage/path/getWritableConfigPath/hashPassword——仅 offline 门控块引用，云端不传安全；hashPassword/getWritableConfigPath 是函数声明提升，可在块1位点注入）。
* **★ 顺手收了 P2-7 残尾**：set-trial-days 的 IPC 早已注释移除，但两端 preload 死桥 `setTrialDays` 一直残留、渲染层零调用；过去靠 main.js `/* */` 注释块里的注册字面量骗过 check-ipc 朴素正则。本次删两端 preload 死桥（getTrialDays 保留），模块改散文注释后假绿灯消失、检查真通过。**教训：安全修复删 handler 必须同步删 preload 桥并 grep 渲染调用点；注释块里的注册字面量会污染朴素静态扫描。**
* **分发/基础设施**：sync-all **Group 22**（$DesktopLicenseIpcTargets=2 electron 目录）+ copy-consistency 新组（**98 副本/22 组/0 失败**）。check-ipc 文本并集扫描不识别 productClass 条件注册——会把同文件内对端分支文本列成 INFO"冗余"（非失败；真实注册面云26/离37 由 smoke 精确证明）。
* **测试资产**：新增 **`tools/desktop-license-smoke.cjs`（127 断言）**——注册面精确集合、分叉两版（试用到期 readOnly/isTrialDenied、云端 readLicense 永久/过期）、E2E 旁路双拦截、gate-failed catch 不 show 主窗+同步抛错重试再败 app.quit（注意：重试 catch 只接同步异常，async reject 在生产走 unhandledRejection 日志——HEAD 既有特性）、register 校验/UPSERT/签名门/registration-info 直通。全门禁 biz-smoke 86/smoke 157/界面 6 OK；**双端真实 Electron 直启 PASS**（云端激活窗/离线登录窗）。
* **三重独立审查（2 功能互盲 + 1 安全，全部并行独立上下文）**：0 阻断/0 重要/0 可利用安全问题；安全结论——productClass 两端硬编码字面量、渲染方数据不进 options；`global.__BNZC_E2E_BYPASS` 仅主进程启动双条件置位、渲染方不可写；register 手机号正则排除原型键、只读具名字段无 spread，signConfig 未生成签名拒绝写盘；fail-closed 语义全部保持。
* **生效方式**：**主进程域热更不可达**——云/离桌面需**重打 exe**（与 P2-3 打印域攒一批，一次双桌面重打即可全部生效）；云端网页/双 APP/鸿蒙不受影响。promo/ 不纳入提交。

## 37. P2 user 域抽 shared/desktop-user-ipc.cjs（2026-09-25，主进程域，密码写点高风险，需双桌面重打 exe）

* **产出**：**`shared/desktop-user-ipc.cjs`（工厂 createDesktopUserIpc(options)）**——双 main.js 用户管理 IPC 收口，**3+1 通道**：通用 2（user:change-password / user:add，两端字节同构）+ 仅离线 1（user:rename-username，productClass==='offline' 门控注册）。hashPassword 本体留 main.js（离线 get-app-config 尚有 2 处调用未迁），以函数声明提升在接线点注入。
* **改前哈希取证（含分节注释整块口径）**：cloud main.js L993-1099 **f323c30c**…、local L1392-1547 **033bee50**…（handler-only 口径 cloud 1f21ae21/local 8a922a05，三个子块哈希见模块头注释）。切片由生成器（tools/_tmp，用后即删）字节组装禁手抄；接线脚本整块替换为 5 行工厂调用，块前后空行接缝校验。
* **★ 本批新坑——真实直启的正确姿势（全记入 `tools/electron-direct-probe.cjs` 头注）**：① `Start-Process -ArgumentList @('.',…)` 会**丢应用路径参数**→electron 落入内置 default_app（其 ESM main 崩溃弹 "Error" 窗挂机 12s，应用根本没跑，此前"存活即通过"是假证据）；② PowerShell `&` 直调 GUI 子系统 exe **不等待/无 stdio/无退出码**，完全不可观测；③ 唯一可靠姿势 = **Node child_process.spawn**；④ 单实例锁冲突会**静默 exit 0**——必须走 E2E 旁路（BNZC_E2E=1 + exe 同级 e2e-enabled.marker + BNZC_E2E_DATA，setPath 在锁之前）+ 跑完删 marker；⑤ **npmmirror 35.0.0 zip dist 的 default_app.asar ESM 崩溃且应用不出窗**——必须用项目自带 dev dist（node_modules/electron/dist/electron.exe，35.7.5），双端均能到「登录/激活窗 showing window」链；⑥ Glob 尊重 .gitignore，查 node_modules 是否存在要用 Test-Path 而非 Glob；⑦ stderr 的 "To load an ES module…" 为 Node22+ require(esm) 兼容提示（HEAD 同样存在）、"wmic 不是内部或外部命令"为本机缺 wmic 噪音，均非异常。
* **回归证据双保险**：stash 前后 HEAD vs 工作区四象限对照（存活/stdout/stderr 全一致）+ dev dist 下双端真实直启（云端"[Cloud] 未激活→login showing window→热更新就绪"、离线"[License] trial 试用模式→login showing window"，均 15s 存活、隔离 userData 内 config 复制/热更缓存零污染工作区）。直启后必查 `git status` 确认热更新缓存未写进源码树。
* **分发/基础设施**：sync-all **Group 23**（$DesktopUserIpcTargets=2 electron 目录）+ copy-consistency 新组（**100 副本/23 组/0 失败**）。`.ps1` 编辑后照例校验 BOM（EF BB BF）完好。
* **测试资产**：**`tools/desktop-user-smoke.cjs`（32 断言全过）**——注册面（云 2/离 3 通道精确集合）、add 校验链（缺字段/短密码/弱密码/用户名格式/查重/默认角色 admin/盐与双哈希字段一致/明文不落盘/signConfig）、change-password 双写路径（config 主路径签名 + systemUsers.json **裸数组**回退不签名 + 用户不存在 + 读异常透因）、rename（用户名定位/手机号定位/phone 保底/携新密码哈希/无账户 synced:false/无文件/异常）。坑：systemUsers.json 按裸数组而非 {users:[]} 读写，冒烟种子别写错形状。
...
## 40. P2 backlog 加固批：打印窗 4 项 + user 6 项 + PBKDF2 慢哈希（2026-09-25，主进程域，需双桌面重打 exe）

* **产出（打印窗，[shared/desktop-print.cjs](file:///d:/trae_projects/kyt-zy/shared/desktop-print.cjs)）**：① handler 入口 `event.senderFrame.isMainFrame` 门控（非主框架直接 false）；② webPreferences 显式 `sandbox:true/webSecurity:true`；③ 收尾改 `destroy()`（防处方 HTML 的 onbeforeunload 阻止 close() 造成窗口残留）；④ 加载前注册 `will-navigate`（preventDefault→safeResolve(false)）+ `setWindowOpenHandler(()=>({action:'deny'}))`；⑤ 打印回调返回真实结果 `safeResolve(success===true)`；⑥ 入参 `String(html==null?'':html)` 归一化防可预防 TypeError。渲染方三个调用方（public/index.html、print-utils.js、print-utils-offline.js）均不消费返回值，契约收紧零影响。
* **产出（user IPC，[shared/desktop-user-ipc.cjs](file:///d:/trae_projects/kyt-zy/shared/desktop-user-ipc.cjs)）**：① 三 handler 全部主框架门控；② change-password 必填旧密码，主进程内置 `verifyStoredPassword` 全格式兼容（PBKDF2/旧全局盐 SHA256/旧用户名盐 SHA256 `bnzc_prescription_salt_v1:<user><pwd>`/历史明文）；改非 admin 目标接受 **admin 密码覆盖**；③ rename 登录窗 frame 仅允许密码同步（忘记密码恢复/登录升级通道），主窗改名/改密必须 oldPassword（验目标）或 adminPassword（验任一 admin）证明；服务端 username+phone 查重（排除自身）+ 格式门（2-30 位 [A-Za-z0-9_-]、非中文、非手机号格式）+ phone 保底；④ user:add 解构白名单删除 role，服务端锁死 `role:'admin'`；⑤ **删除 systemUsers.json 裸数组回退**（全库核实生产无写入路径，属历史死代码）。
* **产出（慢哈希）**：两端 main.js `hashPassword` → **PBKDF2-SHA256 / 600000 轮（OWASP 2025/2026 基线）/ 16B 随机盐**，自描述格式 `pbkdf2_sha256$<iter>$<salt32>$<dk64>`；双 auth-core（offline.js/cloud.js）verifyPassword 新增强格式分支（subtle deriveBits + 常量时间位比较），loginWithUsernamePassword 成功返回 `weakHash` 标记；离线 login.js 登录成功后弱哈希透明升级 config（失败仅 warn 不阻断登录）。渲染层 index.html 旧 SHA-256 hashPassword 残留不影响：UPSERT 方向已核实为 **config→localStorage 单向**，本地旧哈希不会盖回 config。
* **渲染层调用点改动（两权威源手改 → 生成链传播）**：离线 desktop/index.html 的 handleChangePassword 改为**改名也必须输原密码**（旧逻辑允许密码三项全空直接改名）并在 renameUser 调用带 oldPassword；confirmEditUser 管理员给他人改名/改密前 `prompt()` 验证管理员本人密码并带 adminPassword；public/index.html 本地回退分支改名同样要求原密码（云端 API 路径原本已由服务端校验）。同步：sync-all（HTML 生成 + cjs Group 21/23）+ **sync-auth-core.ps1 单独跑**（11 副本，漏跑=打包带旧 auth-core，历史踩坑）。
* **★ 独立三审查抓出的阻断点（已修）**：功能审查发现 `isLoginFrameCall` 旧正则对整串 URL 匹配，`index.html#login.html`（主窗改 hash=同文档导航不重载、不丢脚本上下文）会被误判登录窗 → **主窗无证明重置任意账户密码（含 admin）**；修复为 `new URL(url).pathname` 只判路径（query/hash 均不影响），冒烟新增 3 条伪造回归（#hash / ?query / 真实 login.html?query 仍识别）。安全对抗审查结论 0 Critical/High；其建议的迭代数 120k→**600k 已采纳**（本机全量冒烟 1.3s 无感知）。
...
## 43. 同名诊所多管理员 + 控制台双副本（2026-09-26，小批）

* **同名诊所合并语义**：系统以诊所名称判同一家——两个激活码填同名诊所时，复用同一诊所，首号=诊所管理员、第二号默认医师（2026-08-23 唯一管理员加固）；用户在用户管理点「🔁 角色」可把第二号升为诊所管理员（可并存多个管理员）。
