...
* **双审 P2 遗留（不阻断，后续另开）**：①停用闸 `Array.isArray(clinics)` 对"合法 JSON 但非数组"fail-open（主审核/工单/admin-submit 同形，需四处统一改"非数组即 500"，勿只改一处造成通道分叉）；②工单 pending→approved 无 CAS，platform_admin 双击可双发孤儿码（主审核经 license-write-service 也无终态复核）；③ticket/submit 手机号正则 `[0-9+\-\s]{5,20}` 的 `\s` 含换行，消费面安全但宜收紧。
