...
* 改 index.html JS 后必查（2026-09-13 P1/P2-B1 后新流程）：①改云系 → `sync-html.ps1` 生成 + `html-sync-check.ps1` 绿；②改离线系 → `node tools/sync-index-app.cjs` 生成 + `--verify-only` 绿；③跨版本改动 → `node tools/diff-cross-version.cjs` 双对三层基线绿（desk + siteadmin，单边漏改红灯）；④push 前十道门自动兜底（②含 Group 11+13 生成校验、⑩含跨版本双对）。
