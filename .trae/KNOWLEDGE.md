...
### 2.95【多管理员回归二修】云端权威用户列表——根治存量残留绕过诊所归属过滤（提交 411a706c，2026-08-26）
- **现象**：2.84 诊所归属过滤落地后，云端桌面用户管理再次出现"多个管理员"（王桂杰 13398628756 纯本地残留 + 13398628212 其他诊所账号混入列表）。
- **根因（关键教训）**：2.84 的 `listVisibleUsers` 依赖本地记录 `clinicId` 做归属过滤，但**历史登录落地的存量残留记录本身没有 clinicId**（纯本地账号、其他诊所账号），该函数按"无归属信息不过滤"原则把这些记录放行 → 残留绕过过滤直接显示。这是**基于本地缓存的过滤方案的结构性盲区**：过滤条件存在被过滤的同一条记录上，老数据缺字段就必然漏。不是 2.84 修复丢失，而是它设计的已知局限被真实数据触发。
- **修复方案（云端三端同步 public / cloud_desktop / cloud_app）**：
  1. `fetchCloudClinicUsers`：打开用户管理时调云端 `GET /users`（clinic_admin 返回本诊所权威列表）**直接渲染**——账号归属由云端判定，残留天然不在列表；
  2. `upsertCloudUsersToLocal`：云端列表合并回本地表，补全 `clinicId/clinicName/role/userType`（保留本地 password，离线登录能力不变）——本地记录同步"治愈"，此后本地过滤路径也自然正确；
  3. `refreshUserManageList` 统一刷新入口：云端权威优先，断网/离线回退本地过滤（现状行为，零回归）;
  4. `renderUserList` 参数化：传入云端数组直接渲染；4 处刷新点（打开弹窗/编辑后/添加后/删除后）全部切换。
- **核心设计原则**：**归属判定口径应与记录产生方一致**——云端账号以云端为准，本地表只作缓存/离线容错；不要用可能缺失字段的本机记录做权威过滤。云上资源可达时永远先信云端。
...
### 2.99【修复】只改用户名被"请填写所有字段"挡住——密码三项可全留空（提交 ae55ddb7，2026-08-26）
- **背景**：2.98 上线后用户测试：只填新用户名、密码三项留空 → 被旧的"请填写所有字段"强校验拦截。需求：只改用户名时不填密码，原密码保持不变也可保存。
- **实现要点**（5 端统一）：①`_pwdChange = 密码三项全填`；只填用户名+密码全空放行、半填拦截（提示全填或全留）、全填=纯改密不变；②密码校验段与保存段均包 `if (_pwdChange)`——只改名跳过原密码校验且密码哈希原样保留；③云端账号"只改名"走 set-username（token 认证，成功清登录态回登录页），**不走 change-password**（后端该端点必填密码）；绝不回退本地改名。
- **验证**：语法 5 端与 HEAD 一致；冒烟 5/5；check-interface 基线一致。
- **生效方式**：云端网页版→push 即生效；云端APP→在线资源即生效；云桌面/离线桌面→需重打 exe；离线APP→需重打 APK。

### 2.100【安全】登入框取消"账号已预填"绿色提示——防账号/医师姓名泄露（提交 6b350e80，2026-08-26）
- **背景**：登入框绿色提示"✓ 账号已预填:wgj(医师:赵水云)，请输入密码登录"向任何打开软件的人暴露登录账号+医师真实姓名（离线版还有"密码默认 admin"变体，泄露面更大）。用户明确要求取消（视为对界面保护规则的授权）。
- **实现**：删除 db-offline/desktop 与 cloud_desktop 的 electron/login.js 中 showGreenHint("账号已预填...") 调用（离线版两处含默认密码变体、云端版一处）；**保留**用户名预填、新装试用 admin 密码预填（框内圆点非文字）、密码框自动聚焦、showGreenHint 函数本身（登录成功/激活成功提示仍在用）。APP/网页版无此提示（全局搜索确认）。
...
### 2.104【优化·定版】激活有效期锚定首次激活时间——重激活/换机到期日不变（提交 a1c6bcdb + 0f150a23 后续，2026-08-26）
- **用户规则（定版，覆盖当日早前的"每次激活重算365天"）**：到期时间 = **首次激活时间 + days(默认365)**；不管软件问题/设备问题导致重新激活（同码同设备/换机），到期时间一律不变——防卸载重装反复"续命"。
- **实现**：①validate.js 首次激活写 `firstActivatedAt`（只写一次，重激活/换机不覆盖）；②buildLicenseData 锚定 `firstActivatedAt + days`（旧记录无该字段回退 activatedAt，再无则 now）；③record.expiresAt 保留双用途：码使用期限（validate 过期校验）+ 续费叠加保底（取更晚者，extend.js 语义不破坏）。
- **Node 单测 6/6**：首次=365 / 100天后重激活=265 / 换机重激活=65 / 旧记录回退=165 / 续费剩500=500 / trial 3天后=4。
- **生效方式**：push GitHub 自动部署；全端下次激活即生效（首次激活过的码按 KV 里的 activatedAt 回退锚定）。
...
### 2.110【修复】处方历史栏点击处方误弹"未保存内容"确认框——基线快照对比（提交 a3189bd7，2026-08-28）
- **现象**：云端网页版处方历史栏点击处方后自动弹"当前有未保存的内容，加载历史处方将覆盖。是否继续?"——连续浏览历史处方每次都弹。
- **根因**：loadHistory 加载历史后患者名/诊断/药物被**程序自动填充**，再点另一条历史时这些填充内容被 `(_curName || _curDiag || _hasItems)` 误判为"未保存的编辑"。典型的"程序写界面 ≠ 用户编辑"混淆。
- **修复（7 处副本统一）**：新增 `_presSnapshot()`（患者名+诊断+药物列表 JSON 快照）+ `_updatePresBaseline()`（写 window._presBaseline）；loadHistory 末尾更新基线（加载完成=干净态）；confirm 条件增加 `_dirty`（当前快照≠基线才弹）。基线为 null（从未加载过历史）时保持原保护不降级。
- **效果**：连续浏览历史不弹窗；用户手动新填/改动未保存内容仍弹窗保护；保存成功→clearPrescription 清空表单→点历史本就不弹（空内容短路）。
- **教训（普适）**：**"防丢失"确认框的触发条件必须区分"用户编辑"与"程序填充"**——凡有代码路径会自动填充表单的界面，纯"字段非空"判脏必误报；用基线快照对比（fill/load/save/clear 后更新基线）是通用解法。
- **生效方式**：push GitHub 自动部署（云端网页/云端APP 即时生效）；桌面双版/离线APP 随下次打包自动带上。

### 2.117【推广奖励】用量监控纳入实施：三端邀请码链路全通+管理台用量监控（提交 07b72b10，2026-08-28）
- **背景**：激活用户推广奖励（邀1人+90天封顶4人360天，被邀+30天）服务端已入库，本次补齐客户端链路与用量监控。
- **核心修改**：
  1. 服务端 license-core.js sanitizeRecord 新增 inviteCode/inviteCount/rewardDays/invitedBy（仅管理员API返回，无客户端泄漏）。
  2. **APP端关键bug修复**：LicenseManager 5参 activateOnline 此前接收 inviteCode 参数但从未写入请求体（邀请码永远到不了服务端）——Java 重载链（8参→5参）参数传递正确但实现体内漏 put，本次补上 reqBody.put("inviteCode",...) 并透传返回值 inviteInfo。
  3. APP/桌面共用激活弹窗（shared/auth-core/offline.js + 3处离线副本字节级同步）：新增邀请码输入框+格式校验，按端区分 submit 签名（APP 4参 / 桌面 preload 7参），激活成功提示展示专属邀请码+阶梯进度。
  4. 桌面 activate-window.html 成功页展示邀请码（showSuccess 增 inviteInfo 第3参）。
  5. 管理后台新增「📊 用量监控」Tab（admin-usage API：云端诊所存储/1GB免费额度/写入风险线33家/邀请增长观察+风险预警卡片）+ 激活码列表「邀请进度」列。
- **单测**：邀请奖励核心逻辑 17/17（mock KV 跑 license-core：生成/幂等/查询/发奖/防自邀/防同设备/trial拒绝/无效码/封顶4人360天/到期日锚定叠加/常量）。
- **工作区遗留物清理**：上个丢失上下文会话遗留①约20个JS源文件被混淆工具替换（未提交，HEAD可读源为准）②云端系 auth-core.js 7副本回退到实名防护修复之前的旧版本（回归已提交安全修复）。已全部 `git checkout` 恢复到 HEAD，只保留本次 intentional 改动。**教训：发现工作区有非本会话的大面积异常改动（尤其"已提交修复被回退"），先 diff 确认再恢复，绝不盲目 git add -A。**
- **生效方式**：云端网页/管理后台已推 GitHub 自动部署；云端桌面版无激活窗口无需重打；**离线桌面版需重打 exe；离线APP需重打 APK**（Java桥+auth-core 均有改动）。
...
- **根因**（均为"写了看不到"的显示链路断裂，非功能缺失）：
  1. 版本号：login.html 的 `.version-tag` 初始行内写了 `style="display:none;"`（8-28注释"元素保留视觉隐藏"），`applyEditionTag()` 只写 innerHTML 没改 display → **写了白写**，用户永远看不到。
  2. 下拉按钮：显示条件=`merged.length>0`，数据源=`local_rememberedUsers` 数组 + `config.users`；但 **V1.2.157及之前旧包只写单键`KEY_REMEMBER_USER`用于预填**，不写数组键 → config.users 也为空 → merged.length恒0 → 按钮恒隐藏（用户观感：zsy预填了但没下拉）。
- **修复（桌面两端 login.js 同步，login.html/CSS零改动，check-interface 6/6 OK）**：
  - 版本号闭环：applyEditionTag 首个动作 `tag.style.display='inline-block'`，覆盖 display:none，登录前显示【登录后显示版本】+三元组，登录后显示【云端标准版/机构版】+Vx.x.xx·Arch
  - 下拉按钮闭环：initLoginInput 预填用户名时，单键存在但数组缺失 → **自动反向迁移写入 local_rememberedUsers[0]**，保证首次启动即下拉可见；登录成功 saveRememberedUsername 时已写数组，逻辑一致
- **教训**：所有UI修复都必须按"渲染链路=数据→条件→display"三步逐查；不能只查函数/元素是否存在。"显示条件只依赖某数据键但老用户只存另一个键"是跨版本升级最高频的不可见bug来源。
- **生效方式**：云端桌面版 V1.2.164 已出包（dist）；离线桌面版下次打包带上；网页版/APP本就有版本标签无需操作。

### 2.113【设计重构·定版】"一台设备一版本"拦截式隔离 → 登录放行+降级说明（提交 414529a6 出包 V1.2.163，2026-08-28）
- **背景**：8-18 定版"同一台设备只能激活一个版本"（1f5f68b0），login.js 严格版本匹配拦截登录（机构账户上标准版电脑/反之均报"不能登录"）。用户实际场景反馈：换电脑/换版本即被锁死，体验差，要求按实际场景重设计。
- **新规范（定版）**：**登录=身份认证，授权=功能控制**。
  - 机构账户登录标准版授权电脑 → **放行**+登录成功提示拼接"本机为标准版授权，机构版专属功能不可用，可点激活升级"（user._editionNote 传递，绿色提示不阻断）；
  - 标准账户上机构版电脑 → 静默放行（账户功能是本机子集）。
- **安全防线不降级，收敛为两道真正有效的闸**：①本机功能（用户管理等按钮）由**本机激活码**决定——permission 按 config.edition + enforceEditionBinding 校正逻辑原样保留，未激活机构版本地功能不解锁；②云端数据由**服务端**按账户 clinicEdition/角色鉴权，本地拿不到授权外的机构数据。登录拦截本是冗余防线，删除不破付费墙。
- **改动范围**：桌面两端 login.js 各 1 处（拦截块→_editionNote+放行 + showGreenHint 拼接）；enforceEditionBinding/permission.js 零改动；网页版/APP 本无此拦截（无本机 license 概念）无需动。
- **生效方式**：云端桌面版 V1.2.163 已出包（dist）；离线桌面版修复入库下次打包带上；网页版/APP 无需操作。

### 2.112【修复】登录窗口太小内容溢出+误导性"版本:V1.0.0"文案（提交 60d21f5b 出包 V1.2.162，2026-08-28）
- **现象**：用户截图登录框显示不完全（报错文案"该账户属于【机构版】…"+激活提示被裁切）；且底部"版本: V1.0.0"误导用户误判新旧包（它只是静态版权文案，非程序版本）。
- **根因**：8-26 窗口压缩到 218×330（0338ce8f），报错/激活提示展开后高度溢出；218 宽放不下长报错文案。
- **修复（桌面两端 main.js+login.html 同步）**：①宽度 218→260；②初始高度 330→420；③dom-ready 后 executeJavaScript 读 body.scrollHeight 自适应 setBounds（420-480 区间）；④删除登录框"版本: V1.0.0"静态文案（仅留微信号）；⑤离线版 title 改"- 登录"（与云端一致）。
- **重要甄别结论**：截图报"该账户属于【机构版】不能登录【标准版】电脑"= 用户跑的是**离线桌面版**（标准版电脑+机构版账户被版本隔离拦截，防护正常）。桌面两端登录框 8-28 起已统一同款（diff 仅 CSP/隐藏元素 7 行）；区分靠窗口标题、快捷方式名、登录后主界面顶栏版本。
- **教训**：登录框窗口尺寸改动必须验证"报错提示+激活提示+下拉"全展开场景，不能只看正常态；登录框静态版本文案一律删（版本只在登录后主界面显示，8-23 定版）。
- **生效方式**：云端桌面版 V1.2.162（dist 已出包）；离线桌面版下次打包带上。
